...
`Operation` 列出任务可以执行的文件系统操作。`Copy directory` 复制整个源目录到目标路径。`Copy file` 将源文件复制到指定目标路径。`Create directory` 在指定位置创建目录。`Delete directory` 删除指定目录。`Delete directory contents` 删除指定位置的所有内容。`Delete file` 删除指定文件。`Move directory` 将源目录移动到指定目标。`Move file` 将源文件移动到目标路径。`Rename file` 将源文件重命名为指定目标名称。

`Set attributes` 允许修改指定源文件的属性。

`Hidden` 属性允许指定源文件是否应设置为隐藏。

`ReadOnly` 属性允许设置指定文件的只读属性。`Archive` 属性允许文件被存档。`System` 属性允许将文件设置为系统文件。
