...
某些信息可以存储在项目参数中。因为参数值只能通过 T-SQL 更改，所以参数值几乎是只读的。可以使用 `SSISDB` 中的目录视图和存储过程来查看这些值。`SSISDB` 中的以下对象允许你在执行期间访问参数值：
