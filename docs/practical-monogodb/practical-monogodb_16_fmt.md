...
MongoDB的`fsync`命令允许你在运行 MongoDB 时不更改任何数据的情况下复制数据目录的内容。
...
从3.0版本开始，当使用WiredTiger时，`fsync`不能保证数据文件不会更改。因此，它不能用于确保创建备份时的一致性。

接下来，你将了解从库备份。这是唯一一种能够在没有任何类型停机的情况下获取时间点快照的备份技术。

## 9.2.4 从库备份

从库备份是MongoDB中推荐的数据备份方式。从库存储的数据副本几乎总是与主库同步，并且从库的可用性或性能通常不是问题。你可以在从库而非主库上应用前面讨论的任何技术：关闭、带锁的`fsync`，或转储和还原。
