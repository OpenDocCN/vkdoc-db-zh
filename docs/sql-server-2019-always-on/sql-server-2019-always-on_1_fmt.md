...
构建并配置群集后，就可以安装 SQL Server AlwaysOn 故障转移群集实例了。在我们的场景中，我们希望构建一个跨越我们在第 3 章中构建的 Windows 群集两个节点的群集实例。我们还将讨论如何使用 PowerShell 构建故障转移群集实例。为此，我们需要在群集的主节点上使用“安装 SQL Server 故障转移集群”向导。然后，我们需要运行“添加节点”向导，以允许被动节点在发生故障转移时承载该实例。因此，在本章中，我们将执行以下任务：
...
**清单 5-8** 安装 `sqlserver` PowerShell 模块
...
第四个命令是 `enable`。此命令用于配置集群在节点启动时自动启动。清单 6-20 演示了如何使用此命令。
...
最后一个命令是 `status`。此命令用于检查集群的状态。清单 6-21 显示了此命令的示例输出。
...
现在集群已经构建完成，我们可以配置可用性组了。首先，我们需要创建可用性组。清单 6-22 中的脚本将创建一个名为 `LinuxAOAG` 的可用性组。

```bash
sudo pcs resource create LinuxAOAG ocf:mssql:ag \
    ag_name=LinuxAOAG \
...
接下来，我们需要为可用性组创建一个虚拟 IP 地址。清单 6-23 中的脚本将创建一个名为 `LinuxAOAG_VIP` 的虚拟 IP 地址。

```bash
sudo pcs resource create LinuxAOAG_VIP ocf:heartbeat:IPaddr2 \
    ip=192.168.1.100 \
    cidr_netmask=24 \
...
最后，我们需要将可用性组和虚拟 IP 地址组合到一个资源组中。清单 6-24 中的脚本将创建一个名为 `LinuxAOAG_Group` 的资源组。

```bash
sudo pcs resource group add LinuxAOAG_Group LinuxAOAG LinuxAOAG_VIP
```
...
现在我们已经配置了可用性组，可以测试故障转移了。清单 6-25 中的脚本将模拟主服务器故障。

```bash
sudo pcs node standby LinuxProd
```

清单 6-25：模拟主服务器故障

要验证故障转移是否成功，您可以使用清单 6-26 中的命令检查可用性组的状态。

```bash
sudo pcs resource status LinuxAOAG
```

清单 6-26：检查可用性组状态

要将主服务器恢复在线，您可以使用清单 6-27 中的命令。

```bash
sudo pcs node unstandby LinuxProd
```
...
在本章中，我们学习了如何在 `Linux` 上配置 `SQL Server` 的 `AlwaysOn` 可用性组。我们首先创建了可用性组，然后配置了集群，包括创建登录名、配置防火墙、安装 `Pacemaker` 组件、启用集群服务和构建集群。最后，我们配置了可用性组并测试了故障转移。

通过这些步骤，您可以在 `Linux` 上实现 `SQL Server` 的高可用性和灾难恢复。
