...
如图 10-5 所示，`茂密树`是一种连接树，其连接可能有两个输入都不是表。换句话说，树的结构是完全自由的。查询优化器只有在没有其他可能性时才会选择这种类型的连接树。这通常发生在存在不可合并的视图或子查询时。

![image](img/1005.jpg)

**图 10-5.** *茂密树的结构是完全自由的。*

以下执行计划说明了图 10-5 中描述的连接树。请注意，连接操作 1 的子节点是另外两个连接操作的结果集。
