...
过程的结果（见表`mv_capabilities_table`）显示了物化视图`sales_mv`是否支持三种查询重写模式。如果不支持，`msgtxt`列会指出特定查询重写模式不受支持的原因。在这种情况下，请注意问题是由至少一个仅在`GROUP BY`子句中引用的列引起的。检查SQL语句后，你可以立即识别出该列：`p.prod_status`。
