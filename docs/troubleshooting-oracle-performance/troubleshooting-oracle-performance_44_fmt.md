...
从 Oracle Database 10g 开始，您可以将 SQL 调优委托给查询优化器的一个扩展，称为 `Automatic Tuning Optimizer`。将这个任务委托给最初无法找到高效执行计划的同一个组件，似乎有些奇怪。实际上，这两种情况非常不同。事实上，在正常情况下，查询优化器被限制必须非常快地生成执行计划，通常在亚秒级范围内。相反，可以给 `Automatic Tuning Optimizer` 更多的时间来制定一个高效的执行计划。此外，它还能够使用耗时的技术，例如假设分析，并充分利用动态采样技术来验证其估计。
