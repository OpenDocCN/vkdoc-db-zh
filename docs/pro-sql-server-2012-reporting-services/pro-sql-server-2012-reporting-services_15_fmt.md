...
数据库管理员可以使用标准工具来备份和还原这两个数据库。在 SSRS 初始安装后，可能会添加一个额外的数据库：`RSExecutionLog` 数据库。它存储有关报表执行的更详细信息，例如运行报表的用户、执行时间和性能统计信息。我们将在第 10 章详细介绍创建 `RSExecutionLog` 数据库并讨论报表执行日志记录。
