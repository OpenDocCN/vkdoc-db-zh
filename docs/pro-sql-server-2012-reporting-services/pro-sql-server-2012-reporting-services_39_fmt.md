...
乍一看，很容易发现`Report Builder 1.0`在外观和感觉上类似于 Office 2007 套件中的其他 Microsoft 设计产品，例如`FrontPage`。你会看到一个简单的设计区域、一个`报表数据`区域，以及一个包含可用于每个报表的模板的`报表布局`区域。三个可用模板是`SSRS`报表中可用数据区域的熟悉版本：`表格`、`矩阵`和`图表`。每个模板都有预定义的区域，例如标题、总计列和筛选器描述列，如图 13-23 中的表格报表所示。你还可以在左侧的`报表数据`选项卡中看到`Patient Census`报表模型的字段。
