...
为管理代理创建用户，创建所需目录，并授予管理代理用户权限。假设您正在创建 `ORACLE` 用户，因此请以 `ROOT` 用户登录并运行以下命令：
...
## 2. 登录 OMS 主机并使用 EMCLI 登录 OEM

以 `ORACLE` 用户登录 OMS 主机，并使用 `EMCLI` 登录 OEM：
...
该命令将核心管理代理软件下载到目标目录（`/home/oracle`）。例如，对于 `Linux x86-64`，您将看到文件 `12.1.0.2.0_AgentCore_226.zip`。
...
脚本成功执行后，打开一个新的终端窗口，以 `ROOT` 用户登录，并运行配置脚本（脚本位置写在 `agentDeploy.sh` 的输出中）：
