...
# 使用 EM CLI 管理 Enterprise Manager 目标
...
| 远程主机 | 卸载插件和 sbin 二进制文件。OUI Oracle 主目录移除与 OEM 中目标创建的关系相同。代理主目录是首先创建的，必须作为单独步骤最后删除。 | `cd $AGENT_BASE/core/<agent release>/oui/bin`<br>`./runInstaller -deinstall –remove_all_files` |
