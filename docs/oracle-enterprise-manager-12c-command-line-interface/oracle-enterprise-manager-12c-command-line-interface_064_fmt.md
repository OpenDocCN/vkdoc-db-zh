...
oracle    1723     1  0 16:21 ?        00:00:01 /u01/app/oracle/product/12.1.0/dbhome_1/bin/tnslsnr LISTENER -inherit
```
...
清单 6-11. `updateProps.py` 创建了 `updateProps()` 类
...
清单 6-12 展示了如何使用 `updateProps()` 类来更新一些 EM CLI 目标的属性。该类所做的事情与上一节中的命令完全相同，但你会注意到所需的代码量要少得多。

清单 6-12：导入并使用 `updateProps()` 类来更改目标属性
...
编辑文件 `/u01/app/scripts/upg_agents.txt`，移除版本信息和平台信息，只保留代理主机和端口：
...
```
emcli upgrade_agents -input_file="agents_file:/u01/app/scripts/upg_agents.txt" -job_name="UPG_021814_AGENTS" -stage_location=/u01/app/jobs
```
...
您可以从 EM CLI 对每个已授予 `root`/`sudo` 权限的主机运行 `root.sh`（这是通过 EM12c 环境提供此访问权限的另一个原因）。如果首选凭据没有 `root` 访问权限，则需要通过 EM CLI 或从每个代理目标手动运行它以完成安装。

```
Agent Reason
----- ------
orcl1:3872 Preferred Privileged Credential for Oracle Home of Agent : Not Set | Privilege Delegation for Host : Not Set
```

### 4. 检查升级状态

代理升级后，您将看到以下消息，以便快速从控制台检查作业状态：
...
-------- ------ ------------ ------- -----
CLEANUP_12CR2_AGNTS 成功 2 2013-11-21 17:08:30 CST 2013-11-21 17:08:44 CST
```
