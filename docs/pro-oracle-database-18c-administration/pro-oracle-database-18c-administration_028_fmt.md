...
为了帮助您完全理解此示例，本节展示了用于创建基础表的SQL。假设有两个基础表，定义如下：
...
要确定此物化视图是否可以变得可快速刷新，请使用`DBMS_MVIEW`包的`EXPLAIN_MVIEW`过程的输出。此过程要求您首先创建一个`MV_CAPABILITIES_TABLE`。Oracle提供了一个脚本来完成此操作。请以物化视图所有者的身份运行此脚本：
