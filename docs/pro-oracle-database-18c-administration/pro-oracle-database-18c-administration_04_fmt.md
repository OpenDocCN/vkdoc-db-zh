...
> 提示：有关 Oracle 数据库`DEFAULT`配置文件更改的详细信息，请参阅 MOS 注释 454635.1。
...
你也可以将对象权限授予 `PUBLIC` 用户组（我不推荐这样做）。例如，你可以将表的 `SELECT` 权限授予 `PUBLIC`：

```sql
SQL> grant select on registrations to public;
```

现在，每个用户都可以从 `REGISTRATIONS` 表中进行选择。将对象权限授予 `PUBLIC` 角色几乎总是一个坏主意。作为 DBA，你的主要优先事项之一是确保数据库中的数据安全。将对象权限授予 `PUBLIC` 角色绝对是无法管理谁能访问数据库中什么数据的一种方式。再次强调，不要将对象权限授予 `PUBLIC`。

如果你需要撤销对象权限，请使用 `REVOKE` 语句。此示例从 `INV_MGMT_APP` 用户撤销 DML 权限：
