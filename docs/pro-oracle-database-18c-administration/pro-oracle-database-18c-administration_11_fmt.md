...
`CACHE READS`：Oracle 应在读取时将 LOB 数据置于缓冲区缓存中，但写入时不这样做。

`NOCACHE`：LOB 数据不应置于缓冲区缓存中。这是 SecureFiles 和 BasicFiles LOB 的默认设置。
...
提示：尽量通过 `CREATE TABLE` 语句来启用压缩、去重和加密。如果你使用 `ALTER TABLE` 语句，在修改 LOB 期间表会被锁定。
