...
1. 转到 Players 页面。
2. 点击 `新建部分` 链接。
3. 为 `部分类型` 选择 `文本`，然后点击 `下一步`。
4. 将 `顺序` 设置为 `5`，将 `标题` 设置为 `活跃的注册用户`。
5. 在 `内容` 部分输入以下文本，然后点击 `创建部分` 按钮：
...
2. 点击下拉菜单中的 `访问控制` 选项。
3. 点击 `创建条目` 按钮。
4. 为 `用户名` 输入 `Chris`，并为 `权限` 选择 `读者`。点击 `创建并继续创建另一个` 按钮。
5. 为 `用户名` 输入 `Cameron`，并为 `权限` 选择 `读者`。点击 `创建`。
