...
![img/505695_2_En_2_Fig1_HTML.jpg](img/505695_2_En_2_Fig1_HTML.jpg)
...
如前所述，云的一个重要优势是*弹性扩展*，即能够按需为您的环境增加容量，并尽可能快地利用该额外容量。`Azure Arc-enabled PostgreSQL Hyperscale`突出了这种弹性扩展，因为它可以在支持数据库的`Postgres Worker Nodes`数量上进行扩展（横向扩展），并能将数据分片到这些工作节点及其底层存储上。这实现了并行查询执行和分布式 I/O，从而可以提高数据库容量和性能。
