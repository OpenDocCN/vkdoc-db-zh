...
> **Tip**
>
> 每当获取批量数据时，在`find()`操作中包含投影。投影减少了 MongoDB 需要通过网络传输的数据量，因此可以减少网络往返。
...
在下面的例子中，查询中没有指定投影，所以我们不能期望看到`PROJECTION_COVERED`。相反，我们有一个`FETCH`操作——但是请注意，`FETCH`中处理的行数与`IXSCAN`中的文档数完全相同——这表明索引检索到了我们需要的所有文档。
