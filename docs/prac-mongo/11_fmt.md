...
数据目录中的文件命名为`<dbname>.0`、`<dbname>.1`等。`mongod`分配的第一个文件的大小是 64MB，所有后续文件的大小都会增加 2 倍，因此第二个文件将为 128MB，第三个文件将为 256MB，依此类推，直到达到 2GB，之后所有文件的大小都将为 2GB。虽然空间是在创建时分配给数据文件的，但可能有 90%的文件是空的。对于较大的数据库，这种未使用的分配空间通常很小。

*   此选项可以通过使用`--noprealloc`选项禁用。但是，不建议在生产环境中使用它。它应该仅用于测试和频繁调用的小数据集的 drop 数据库。

*   OP: 如果`mongod`是副本集的成员，数据目录中将有一个名为`oplog.rs`的文件。该文件存在于本地数据库中，是一个具有上限的预分配集合。在 64 位安装中，此文件的分配默认约为磁盘空间的 5%。

*   Journal: 日志文件也包含在数据目录中，它们在 MongoDB 将日志文件应用于数据库之前，将写操作存储在磁盘上。

*   MongoDB 为日志预分配了 3GB 的数据，这超过了实际数据库的大小，因此不适合小规模安装。解决此问题的可用解决方案是在命令行标志或`/etc/mongod.conf`文件中使用`--smallflags`，直到您在具有所需磁盘空间的环境中运行。但此功能使其不适合小安装。

*   Empty record: 当文档或集合被删除时，空间永远不会返回给操作系统；相反，MongoDB 维护这些空记录的列表，这些记录可以被重用。
...
索引是内存密集型的；换句话说，索引占用大量 RAM。由于这些是 B-tree 索引，定义许多索引会导致系统资源更快地消耗。其结果是，当需要时内存会自动分配。在共享环境中，运行数据库会更棘手。一般来说，与所有数据库服务器一样，最好在专用服务器上运行 MongoDB。
...
如果`max`参数用于指定封顶集合中的最大文档数，则不能超过 2^32 个文档。但是，如果没有使用这样的参数，文档的数量就没有限制。
