...
4. 运行 `Step 7`，查看一个扫描外部表中所有行的简单示例。在此示例中，如果查询成功，应该只返回一行：
...
5. 运行 `Step 8`，使用 WHERE 子句筛选结果（并可能使用向下推送到外部数据源）：
...
6. 运行 `Step 9`，使用 UNION 查找 SQL Server 2019 和 Azure SQL 数据库上的所有库存物品：
