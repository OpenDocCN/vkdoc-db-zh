...
虽然`Azure 门户`提供了观察`DTU`使用情况的机制，但并未提供建立基准线的机制。相反，您应使用`Azure SQL Database`特定的DMV `sys.dm_db_resource_stats`。此DMV维护有关给定`Azure SQL Database`的`DTU`使用情况的信息。它包含以15分钟为间隔汇总的一小时信息。要像对待`SQL Server`实例一样建立基准线，您需要随时间捕获此数据。将`sys.dm_db_resource_stats`中显示的信息收集到表中，将是您为`Azure SQL Database`的性能指标建立基准线的方式。
