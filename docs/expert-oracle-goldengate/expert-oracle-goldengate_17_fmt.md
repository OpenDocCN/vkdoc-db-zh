...
此作业的命令行也在图 9-5 中展示，对于 Windows 系统，命令为 `vericon.exe –job job_big_table`。点击 `Run Job` 按钮来运行作业。如果作业正在运行，`Run Job` 按钮的文本会变为 `Stop Job`。如果你想停止作业，可以点击它。你也可以前往 `Running Jobs` 或 `Finished Jobs` 面板查看状态。如果出现任何错误，你可以深入查看组和比较对以获取详细的错误信息。
