...
当你深入研究此控件的代码时，将开始看到通过使用抽象类，控件所能利用的所有功能。就在几年前，创建控件还相当困难。在 ASP.NET 2.0 中，随着 `CompositeControl` 和 `CompositeDataBoundControl` 的出现，这变得容易多了，标准 ASP.NET 控件正是构建于这些基类之上。你也可以使用这些基类，它们自动为你提供丰富的设计时支持以及广泛的运行时功能。
