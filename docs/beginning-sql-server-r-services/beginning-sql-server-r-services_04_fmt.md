...
提示：如果您没有以本地 `Administrator` 帐户登录，您可能需要右键单击“开始”菜单中出现的桌面应用图标，然后选择 `以管理员身份运行`。
...
完成安装的最后一步是安装 Microsoft R Open。请返回到 Microsoft 提供并显示在图 2-1 中的网页，点击 **Download Microsoft R Open** 链接。此时，会打开另一个网页；其 URL 是 `https://mran.revolutionanalytics.com/download/`。图 2-14 显示了此时您应该看到的 Microsoft R Open 下载页面。
