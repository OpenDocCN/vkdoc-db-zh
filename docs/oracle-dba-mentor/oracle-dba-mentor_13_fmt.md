...
Oracle 为我们提供了一个名为 LiveSQL 的测试平台。这个平台是一个基于 Web 的查询工具，与一个真实的 Oracle 数据库交互。任何人都可以通过将 Web 浏览器指向 `http://livesql.oracle.com` 来访问 LiveSQL 并开始使用。在右上角，单击 Sign In 链接，使用相同的 Oracle 凭据访问 Oracle Technology Network 或 My Oracle Support。在左侧，单击 SQL Worksheet 项，如图 10-7 所示。
