...
让我们检查数据库服务器上`Listener`的状态。检查状态的命令是`lsnrctl status`，如清单 9-6 所示。
...
端口下面的那行显示`Listener`当前不支持任何服务。此时没有服务的原因是服务器上没有运行`Oracle`实例。启动`Oracle`实例后，我们可以再次检查状态，如清单 9-7 所示。
