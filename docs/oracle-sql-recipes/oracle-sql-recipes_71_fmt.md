...
| 排序合并连接 | 将来自两个独立表或结果集的行进行连接。当行已排序时，排序合并连接效率较高。哈希连接通常比排序合并连接更高效。 |
| 笛卡尔连接 | 在没有连接条件时使用。 |
| 外连接 | 返回其中一个表中不满足连接条件的行。 |
...
3.  **启用对象审计**
    ...
    现在，您可以按对象、权限或语句启用审计。例如，以下语句对 `INV_MGMT` 用户拥有的 `EMP` 表上的所有 DML 访问启用审计：
...
- **桶**，将结果排序至等大小的桶中，用于处理 `string`内的字符替换, 172—174
- `repo`索引, 271—273
- 构建聚合函数，39—42
...
- 两张表的内容，同步操作, 138—140
- 创建操作, 105—107
- `COMMIT`语句，以及`DML`语句，209
- 转换
- 字符串与数值数据类型之间，187—188
- 比较
- 数据集上的假设, 277—279
- 时区间的日期和时间，152—153
- 连接条件中的`NULL`值, 80—81
...
- 相关更新, 79
- 层次数据与, 314
- `COUNT(DISTINCT ... )`聚合函数，164—165
- 使用指定层次查询, 315—317
- `COUNT`函数, 24, 34—35
- `CONNECT_BY_ISCYCLE`伪列，314, 329—330
- `COUNT(*)`函数, 25
- 计数
- `CONNECT_BY_ISLEAF`伪列，315, 324—328
- 十二进制, 143
...
`INDEXES clause` 用于, 381

`LAST_DAY function`, 155—156

在连接条件中处理 `NULLs`, 80—81
...
`MERGE PARTITIONS clause` (`ALTER TABLE statement`), 373—374
