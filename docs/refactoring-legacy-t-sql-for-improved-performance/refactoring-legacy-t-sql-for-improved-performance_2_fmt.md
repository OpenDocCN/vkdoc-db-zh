...
Listing 1-5 更新用户 ID 为 22656 的显示名称
...
表 1-2 代码清单 1-5 对应的 Statistics Parser 网站输出的“总计”部分读取列
...
Listing 1-6 在 `WidePosts` 表上创建索引的 `CREATE` 语句
...
Listing 1-8 更新拥有大量 `WidePosts` 行的用户

`STATISTICS IO` 和 `TIME` 的输出如表 1-4 所示。

表 1-4 更新拥有大量 `WidePosts` 行的用户时的 `STATISTICS IO` 和 `TIME` 输出
