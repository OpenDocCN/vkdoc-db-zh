...
电子补充材料 本章的在线版本（doi:`10.​1007/​978-1-4842-1811-2_​1`）包含补充材料，可供授权用户使用。
...
1.  创建一个用于存储动态 SQL 的字符串变量。可以使用任何变量名。
2.  构建命令字符串并将其存储在此变量中。
3.  执行命令字符串。
...
清单 1-2. 为备份所有以“AdventureWorks”开头的数据库而构建的动态 SQL
