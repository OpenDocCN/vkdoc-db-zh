...
图 7-9 挂载到 Linux 容器上 SQL Server 的卷的内容
...
图 7-10 在卷中创建的 SQL Server 数据库文件
...
图 7-11 在卷中创建的 SQL Server 用户数据库
...
图 7-12 在卷中创建的 SQL Server 用户数据库 - Windows 容器

## 自定义默认数据库位置

注意：你可以修改在容器上运行的 SQL Server 的默认 SQL Server 数据库文件位置——无论是 Windows 还是 Linux。你可以通过创建自定义 SQL Server 镜像来实现，而不是使用 Microsoft 提供的公开镜像。第 9 章和第 10 章分别介绍了如何为 Windows 和 Linux 创建自定义 SQL Server 镜像。
