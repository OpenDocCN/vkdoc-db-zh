...
此 SQL 查询使用一个名为`CustomerTotals`的公共表表达式（CTE）来以结构化方式计算并组织客户购买数据。该 CTE 首先通过汇总`Sales`表中的`Sale_Amount`并按客户和地区分组，为每个客户`Customer_ID`在每一个地区`Region`聚合总购买金额`Total_Purchase`。此步骤将聚合逻辑分离出来，生成一个简洁的结果集，其中包含每个客户在每个地区的一行记录，汇总了他们的总消费。
