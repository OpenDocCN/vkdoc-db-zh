...
| 函数 | 描述 | 示例 |
| --- | --- | --- |
| `SQRT(x)` | 返回 `x` 的平方根。 | `SELECT SQRT(16);` |
| `TRUNC(x)` | 向零截断 `x`。 | `SELECT TRUNC(4.8);` |
| `TRUNC(x, s)` | 将 `x` 截断到 `s` 位小数。 | `SELECT TRUNC(4.567, 2);` |
| `RANDOM()` | 返回 0 到 1 之间的伪随机数。 | `SELECT RANDOM();` |

### 统计函数

| 函数 | 描述 | 示例 |
| --- | --- | --- |
| `AVG(expression)` | 返回输入值的平均值（算术平均）。 | `SELECT AVG(salary) FROM employees;` |
| `CORR(y, x)` | 返回相关系数。 | `SELECT CORR(sales, advertising) FROM marketing;` |
| `COVAR_POP(y, x)` | 返回总体协方差。 | `SELECT COVAR_POP(sales, advertising) FROM marketing;` |
| `COVAR_SAMP(y, x)` | 返回样本协方差。 | `SELECT COVAR_SAMP(sales, advertising) FROM marketing;` |
| `STDDEV_POP(expression)` | 返回总体标准差。 | `SELECT STDDEV_POP(salary) FROM employees;` |
| `STDDEV_SAMP(expression)` | 返回样本标准差。 | `SELECT STDDEV_SAMP(salary) FROM employees;` |
| `VARIANCE(expression)` | 返回样本方差。 | `SELECT VARIANCE(salary) FROM employees;` |
| `VAR_POP(expression)` | 返回总体方差。 | `SELECT VAR_POP(salary) FROM employees;` |
| `VAR_SAMP(expression)` | 返回样本方差。 | `SELECT VAR_SAMP(salary) FROM employees;` |
| `REGR_AVGX(y, x)` | 返回自变量 (`x`) 的平均值。 | `SELECT REGR_AVGX(y, x) FROM table;` |
| `REGR_AVGY(y, x)` | 返回因变量 (`y`) 的平均值。 | `SELECT REGR_AVGY(y, x) FROM table;` |
| `REGR_INTERCEPT(y, x)` | 返回最小二乘回归线的 y 截距。 | `SELECT REGR_INTERCEPT(y, x) FROM table;` |
| `REGR_R2(y, x)` | 返回相关系数的平方。 | `SELECT REGR_R2(y, x) FROM table;` |
| `REGR_SLOPE(y, x)` | 返回最小二乘回归线的斜率。 | `SELECT REGR_SLOPE(y, x) FROM table;` |
| `REGR_COUNT(y, x)` | 返回两个表达式都不为 null 的输入行数。 | `SELECT REGR_COUNT(y, x) FROM table;` |
