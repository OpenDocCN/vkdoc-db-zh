...
| 类别 | 基本语法 |
| --- | --- |
| 行号 | `SELECT column1, ROW_NUMBER() OVER (ORDER BY column2) AS row_num FROM table_name;` |
| 按分区 | `SELECT column1, column2, ROW_NUMBER() OVER (PARTITION BY column1 ORDER BY column2) AS row_num FROM table_name;` |
| 常见窗口函数 | `SELECT column1, column2, SUM(column2) OVER (PARTITION BY column1) AS sum, AVG(column2) OVER (PARTITION BY column1) AS avg, MIN(column2) OVER (PARTITION BY column1) AS min, MAX(column2) OVER (PARTITION BY column1) AS max, COUNT(*) OVER (PARTITION BY column1) AS count FROM table_name;` |
| 排名函数 | `SELECT column1, column2, ROW_NUMBER() OVER (ORDER BY column2) AS row_num, RANK() OVER (ORDER BY column2) AS rank, DENSE_RANK() OVER (ORDER BY column2) AS dense_rank, NTILE(4) OVER (ORDER BY column2) AS quartile FROM table_name;` |
| 前导和滞后函数 | `SELECT column1, column2, LEAD(column2) OVER (ORDER BY column1) AS next_value, LAG(column2) OVER (ORDER BY column1) AS prev_value FROM table_name;` |
...
| 类别 | 基本语法 |
| --- | --- |
| 当前日期和时间 | `SELECT CURRENT_DATE;` -- 当前日期<br>`SELECT CURRENT_TIME;` -- 当前时间<br>`SELECT CURRENT_TIMESTAMP;` -- 当前日期和时间<br>`SELECT NOW();` -- 当前时间戳 |
| 日期/时间运算 | `SELECT date_column + INTERVAL '1 day';` -- 加 1 天<br>`SELECT date_column + INTERVAL '2 hours';` -- 加 2 小时<br>`SELECT date_column + INTERVAL '3 months';` -- 加 3 个月<br>`SELECT date_column - INTERVAL '1 year';` -- 减 1 年 |
| 日期/时间提取 | `SELECT EXTRACT(YEAR FROM date_column);` -- 获取年份<br>`SELECT EXTRACT(MONTH FROM date_column);` -- 获取月份<br>`SELECT EXTRACT(DAY FROM date_column);` -- 获取日<br>`SELECT EXTRACT(HOUR FROM timestamp_column);` -- 获取小时<br>`SELECT EXTRACT(DOW FROM date_column);` -- 星期几 (0-6) |
| 日期/时间格式化 | `SELECT TO_CHAR(date_column, 'YYYY-MM-DD');` -- 格式化日期<br>`SELECT TO_CHAR(timestamp_column, 'HH24:MI:SS');` -- 格式化时间 |
...
| 类别 | 类型 | 基本语法 |
| --- | --- | --- |
| 条件表达式 | `CASE` 表达式 | `SELECT column1, CASE WHEN condition1 THEN result1 WHEN condition2 THEN result2 ELSE result3 END AS new_column FROM table_name;` |
...
| 类别 | 类型 | 基本语法 |
| --- | --- | --- |
| 创建视图 | 基本视图创建 | `CREATE VIEW view_name AS SELECT column1, column2 FROM table_name WHERE condition;` |
|  | 创建或替换现有视图 | `CREATE OR REPLACE VIEW view_name AS SELECT column1, column2 FROM table_name WHERE condition;` |
|  | 物化视图 (物理存储，需要刷新) | `CREATE MATERIALIZED VIEW mat_view_name AS SELECT column1, column2 FROM table_name WHERE condition;` |
|  | 带选项创建 | `CREATE VIEW view_name WITH (security_barrier=true) AS SELECT column1, column2 FROM table_name WHERE condition;` |
...
|  | 与视图进行连接 | `SELECT t.column1, v.column2 FROM table_name t JOIN view_name v ON t.id = v.id;` |
...
| 索引信息 | 列出数据库中所有索引 | `SELECT indexname, tablename, indexdef FROM pg_indexes WHERE schemaname = 'public' ORDER BY tablename, indexname;` |
|  | 列出特定表上的索引 | `SELECT indexname, indexdef FROM pg_indexes WHERE tablename = 'table_name';` |
...
SELECT
    ...
    index.relname AS index_name,
    idx_scan AS times_used,
    idx_tup_read AS tuples_read,
    idx_tup_fetch AS tuples_fetched
FROM pg_stat_user_indexes idxstat
JOIN pg_stat_user_tables tabstat ON idxstat.relname = tabstat.relname
WHERE idxstat.relname = 'table_name'
