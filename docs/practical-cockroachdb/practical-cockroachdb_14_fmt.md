...
如果你正在索引包含顺序数据的列，例如 `TIMESTAMP` 或递增的 `INT` 值，你可能需要考虑使用哈希分片索引。
...
接下来，我们将创建视图。这个视图将是对 `purchase` 表的一个简单选择，并按从 `checkout_at` 列提取的星期几的值进行分组：
