...
清单 15-10 添加 `DecodeLoggingLevel` 函数
...
清单 15-11 编辑 `returnExecutionValueParameterSet` 函数
...
图 15-17 编辑后的 `returnExecutionValueParameterSet` 函数
...
清单 15-12 向 `SettingsNode` 添加 `LoggingLevel` 属性
...
图 15-18 已实现的 `SettingsNode.LoggingLevel` 属性
...
图 15-19 将 `LoggingLevel` 属性设置为 `Verbose`
...
图 15-21 `LoggingLevel` 执行参数设置为 `3` (`Verbose`)
