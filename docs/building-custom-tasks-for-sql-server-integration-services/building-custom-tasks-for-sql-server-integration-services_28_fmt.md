...
存储过程 `xp_msver` 在本地 SQL Server 实例中可用。存储过程 `xp_msver` 在 Azure SQL 托管实例中可用。在撰写本文时，存储过程 `xp_msver` 在 Azure SQL DB 实例中不可用。
...
清单 25-8 将 `xp_msver` 存储过程添加到 `SSISDB` 数据库
...
您还可以看到`RunForSomeTime.dtsx`的执行不等待`ReportParameterAndSucceed.dtsx`执行完成。`ReportParameterAndSucceed.dtsx`执行了近 13 秒（参见“持续时间”列）。
