...
打开 SQL Run for some time 执行 SQL 任务编辑器，并将 `ConnectionType` 设置为 `ADO.NET`。单击 `Connection` 下拉菜单，然后单击“<New connection…>”。配置一个 `ADO.Net` 连接管理器到任何数据库——此包将不会使用配置的连接，但 `Execute SQL Task` 需要配置一个连接管理器。
