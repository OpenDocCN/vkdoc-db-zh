...
**清单 13-2** 实现 GeneralView 的 OnInitialize 方法
...
**图 13-2** GeneralView 的 OnInitialize 方法
...
**图 13-3** 添加 “using Microsoft.SqlServer.Dts.Runtime;”
...
清单 13-22 向 `ADONetConnections` 添加 `TypeConverter` 方法
...
开发至此真是太棒了：我们已经成功暴露了 `SourceConnection` 属性！这是个好消息。坏消息是，`SourceConnection` 属性目前还没有任何作用。不过，我们已经非常接近了。
