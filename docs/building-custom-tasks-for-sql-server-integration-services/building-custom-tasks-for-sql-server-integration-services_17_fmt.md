...
**清单 14-3** 响应 `SourceConnection` 属性变更
...
**图 14-3** 仅响应 `SourceConnection` 属性变更
...
**清单 14-4** 向 `SettingsView` 添加 `NEW_CONNECTION` 字符串常量
...
**图 14-4** 在 `SettingsView` 中声明 `NEW_CONNECTION` 常量
...
**清单 14-6** 向 `SettingsView` 添加 `connectionService`
...
**图 14-6** 已将 `connectionService` 添加到 `SettingsView`
...
**清单 14-7** 在 `OnInitialize` 中初始化 `connectionService`
...
**图 14-7** 在 `SettingsView OnInitialize` 中初始化了 `connectionService`
