...
当您发出 `RESTORE DATABASE` 命令时，RMAN 会随数据库的其余部分一起恢复只读表空间。例如，以下命令将恢复所有数据文件（包括处于只读模式的数据文件）：
...
根据为故障组在 `V$LOG` 中报告的状态，使用表 7-1 来确定应采取的操作。
...
SQL> alter database add logfile member '/u02/oraredo/O12C/redo02b.rdo' to group 2;
...
```
SQL> alter database add logfile member '</directory/file_name>' reuse to group <group#>;
```
