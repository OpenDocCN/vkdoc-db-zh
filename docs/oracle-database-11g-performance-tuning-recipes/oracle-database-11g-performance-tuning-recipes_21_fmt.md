...
`ALL` 操作符实质上使用逻辑 `AND` 操作符来比较查询中列出的值。使用 `ANY` 操作符时，每个值都是单独比较以查看是否存在匹配项，而 `ALL` 操作符需要比较列表中的每个值才能确定是否存在匹配项。以我们的部门表为例，请看以下查询。在此查询中，如果 `DEPARTMENT_ID` 的值小于或等于列表中所有值，我们将从表中检索部门名称：
