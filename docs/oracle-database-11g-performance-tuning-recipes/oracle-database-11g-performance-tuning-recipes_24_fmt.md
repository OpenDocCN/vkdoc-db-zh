...
在应用程序中实现绑定变量可以极大地提高查询的效率和性能。本质上，绑定变量也称为替换变量，用于替换查询中的字面值。通过将绑定变量放入 SQL 语句中，这些语句可以在内存中被重复使用，从而无需经历整个昂贵的 SQL 解析过程。
...
## 使用 `V$SQL_PLAN_MONITOR` 和 `DBMS_SQLTUNE.REPORT_SQL_MONITOR` 监控查询进度
...
## Global Stats
| Elapsed Time(s) | Cpu Time(s) | IO Waits(s) | Concurrency Waits(s) | Other Waits(s) | Fetch Calls | Buffer Gets | Read Reqs | Read Bytes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
