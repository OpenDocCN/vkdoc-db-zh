...
索引提示的其他例子包括用于索引快速全扫描的 `INDEX_FFS` 和用于索引跳跃扫描的 `INDEX_SS`。如果您有一个包含复合、多列索引的表，`INDEX_SS` 提示是合适的。即使查询不使用索引的前导列，也可能让 Oracle 使用该索引。有时，即使 `WHERE` 子句中提到的列不是索引的前导列，`INDEX_SS` 提示也可以有益于快速检索数据。例如，如果我们想获取所有获得佣金的员工的名字，我们的查询将如下所示：
