...
要创建一个 SQL 测试案例，首先必须导出 SQL 语句以及关于该语句的若干有用信息。以下示例展示了如何捕获抛出错误的 SQL 语句。在此示例，用户 SH 执行导出操作（你无法以用户 SYS 的身份执行导出）。
...
5.  选择要从中选择第二个时段快照对的天数。输入值 4，以便从过去 4 天中选择一对快照。
    ```
    指定要选择的快照天数
    输入 num_days 的值: 4
    ```
