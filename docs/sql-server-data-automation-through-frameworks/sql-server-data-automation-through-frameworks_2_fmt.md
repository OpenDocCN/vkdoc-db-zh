...
一旦确定解决方案，我们可以从消息中看出`FWDemo.DailyProcess1`已经成功运行，因此无需重新运行。我们在消息中看到`ApplicationProcessProcedureID`值是1。我们可以使用代码清单4-11中的代码来禁用`FWDemo.DailyProcess1`，以便在应用程序执行时它不会运行。
...
此时，可以像之前一样重新启动计划任务，执行将从`FWDemo.DailyProcess2`开始，如代码清单4-12所示。任务完成后，只需使用类似代码清单4-13中的代码重置`FWDemo.DailyProcess1`的`Active`列。
