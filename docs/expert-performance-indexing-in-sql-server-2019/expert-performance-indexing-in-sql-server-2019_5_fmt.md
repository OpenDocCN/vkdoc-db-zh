...
SELECT * FROM `sys.dm_db_database_page_allocations` (`{database_id}`, `{TableId | NULL}`, `{IndexId | NULL}`, `{ PartitionId | NULL }`, `{DETAILED | LIMITED}`)
...
| `table_name | table_object_id` | 通过提供表名或表的 `object_ID` 来指定输出中返回哪个表。如果未提供值，输出将包含所有表的结果。 |
| `index_name | index_id` | 通过提供索引名称或 `index_ID` 来指定输出中返回哪个索引。如果提供 -1 或未提供值，则输出将包含该表上所有索引的结果。 |
...
图 2-28 转发记录前 `dbo.HeapForwardedRecords` 的物理状态
...
图 2-29 转发记录后 `dbo.HeapForwardedRecords` 的物理状态
...
表 3-3：`DBCC SHOW_STATISTICS` 中的直方图列
...
图 3-3：`Sales.SalesOrderDetail` 表上索引的直方图示例
...
![针对 `dbo.SalesOrderHeaderStats` 的示例查询，`sys.dm_db_stats_properties` 的查询结果](img/338675_3_En_3_Fig6_HTML.jpg)
**图 3-6**
...
表 3-8 `sys.dm_db_index_usage_stats` 中的标题列
...
图 3-8 `sys.dm_db_index_usage_stats` 标题列查询结果
...
代码清单 3-12 查询 `sys.dm_db_index_usage_stats` 中标题列的语句
...
![扫描查询的查询计划](img/338675_3_En_3_Fig11_HTML.jpg)
图 3-11 扫描查询的查询计划
...
![index_scans 的查询结果](img/338675_3_En_3_Fig12_HTML.jpg)
...
![查找和键查找的查询计划](img/338675_3_En_3_Fig13_HTML.jpg)
图 3-13 查找和键查找的查询计划
...
![index_lookups 的查询结果](img/338675_3_En_3_Fig14_HTML.jpg)
...
![index_updates 的查询结果](img/338675_3_En_3_Fig15_HTML.jpg)
...
**表 3-11**: `sys.dm_db_index_operational_stats` 的参数
...
表 3-12 `sys.dm_db_index_operational_stats` 中的头列
...
表 3-13 `sys.dm_db_index_operational_stats` 中的 DML 活动列
...
图 3-17 数据操作语言活动查询结果（结果在您的系统上可能有所不同）
...
![行锁的查询结果](img/338675_3_En_3_Fig21_HTML.jpg)
...
![页锁的查询结果](img/338675_3_En_3_Fig22_HTML.jpg)
...
清单 5-5：使用 `MakeValid()` 纠正任何无效的 `GEOMETRY` 实例
...
**图 5-4**：使用 `STDistance()` 缩小邮政编码数据的结果
...
![具有哈希索引的内存优化表查询的持续时间](img/338675_3_En_6_Fig1_HTML.jpg)

图 6-1 具有哈希索引的内存优化表查询的持续时间
...
![具有哈希索引的内存优化表查询的持续时间](img/338675_3_En_6_Fig2_HTML.jpg)

图 6-2 具有哈希索引的内存优化表查询的持续时间
...
代码清单 7-7 使用 `sys.fulltext_stoplists` 查看停用词列表单词
...
![INSERT 后的碎片化结果](img/338675_3_En_9_Fig2_HTML.jpg)
...
![非聚集索引碎片化结果](img/338675_3_En_9_Fig3_HTML.jpg)
...
![非聚集索引在 INSERT 后的碎片化结果](img/338675_3_En_9_Fig4_HTML.jpg)
...
![初始的 UPDATE 碎片化结果](img/338675_3_En_9_Fig5_HTML.jpg)
...
![表碎片数据的查询结果](img/338675_3_En_9_Fig24_HTML.jpg)
...
![用于重建/重组操作的索引](img/338675_3_En_9_Fig25_HTML.jpg)
...
下一个 DMO 是 `sys.dm_db_missing_index_columns`，它是一个动态管理函数。此函数为 `sys.dm_db_missing_index_details` 中列出的每个缺失索引返回一个列列表。要使用此 DMF，需将一个 `index_handle` 作为参数传入函数。结果集中的每一行都代表来自 `sys.dm_db_missing_index_details` 的缺失索引建议中的一个列，并重复 `equality_columns`、`inequality_columns` 和 `included_columns` 中的信息。表 10-2 列出了 `sys.dm_db_missing_index_columns` 的输出。
...
为了解释缺失索引 DMO 如何工作以及它们之间的关系，我将带您看一个包含几条 SQL 语句的示例。这些语句如清单 10-1 所示，针对 `AdventureWorks2017` 数据库中的 `SalesOrderHeader` 表执行了几个查询。每个查询的过滤条件都基于 `DueDate` 或 `OrderDate` 列，或同时基于两者。
...
![图 11-18 Search Columns 模式的执行计划](img/338675_3_En_11_Fig18_HTML.jpg)

![图 11-17 Search Columns 模式的 Statistics I/O 结果](img/338675_3_En_11_Fig17_HTML.jpg)
...
*清单 11-15 Search Columns Pattern*
...
![图 11-20 Index Intersection 模式的执行计划](img/338675_3_En_11_Fig20_HTML.jpg)

![图 11-19 Index Intersection 模式的 Statistics I/O 结果](img/338675_3_En_11_Fig19_HTML.jpg)
...
*清单 11-16 Index Intersection Pattern*
...
![图 11-21](img/338675_3_En_11_Fig21_HTML.jpg)
...
![图 11-23 索引交叉模式的执行计划](img/338675_3_En_11_Fig23_HTML.jpg)

![图 11-22 索引交叉模式的统计信息 I/O 结果](img/338675_3_En_11_Fig22_HTML.jpg)
...
**图 11-26** 覆盖索引模式的 Statistics I/O 结果
...
**图 11-28** 覆盖索引模式的 Statistics I/O 结果
...
`包含列`模式是覆盖索引模式的近亲。`包含列`模式利用了 `CREATE` 和 `ALTER INDEX` 语法中的 `INCLUDE` 子句。该子句允许将非键列添加到非聚集索引中，类似于非键数据存储在聚集索引上的方式。这是`包含列`模式和覆盖索引模式之间的主要区别，在覆盖索引模式中，附加列是索引的键列。与聚集索引类似，作为 `INCLUDE` 子句一部分的非键列不会被排序，尽管它们可以在某些查询中用作谓词。

`包含列`模式的用例来自于它提供的灵活性。它的用法通常与覆盖索引模式相同，有时这两个名称可以互换使用。关键区别（将在本节中演示）在于，覆盖索引模式受限于索引中所有列的排序顺序。`包含列`模式可以通过包含非键数据来避免这个潜在问题，从而增加其使用的灵活性。

在演示`包含列`模式的灵活性之前，让我们首先检查针对 `dbo.Contacts` 表的另一个索引。在代码清单 11-21 中，查询仅筛选 `FirstName` 值为 Catherine，并返回 `ContactID`、`FirstName`、`LastName` 和 `EmailAddress` 列。此查询请求与其他示例不同，因为它现在包含了 `EmailAddress` 列。由于此列未包含在任何其他非聚集索引中，因此没有索引能完全满足该查询。结果，执行计划使用 `IX_Contacts_FirstName` 来识别 Catherine 的行，然后从聚集索引中查找其余数据，如图 11-30 所示。由于进行了键查找，查询的读取次数也增加到 68 次读取（参见图 11-31），这与之前的示例一样。
...
为了提高此查询的性能，可以基于多列模式或覆盖索引模式创建另一个索引。然而，这些选项的问题在于，最终生成的索引将与它们所能改进的查询具有相同的限制。相反，我们将创建一个基于`包含列`模式的新索引。这个新索引，如代码清单 11-22 所示，以 `FirstName` 作为键列，并包含 `LastName`、`IsActive` 和 `EmailAddress` 作为非键列。尽管 `IsActive` 列未在索引中使用，但将其包含在内是为了允许索引具有额外的灵活性，本节后面的一个示例将利用这一点。索引就位后，代码清单 11-22 中查询的性能显著提高。在此示例中，每次执行的读取次数从之前的 68 次下降到 3 次（参见图 11-32）。在执行计划中，不再需要键查找和嵌套循环；取而代之的只是索引查找，现在使用的是索引 `IX_Contacts_FirstNameINC`（参见图 11-33）。
...
![外键关系](img/338675_3_En_11_Fig41_HTML.jpg)
...
![外键模式的执行计划](img/338675_3_En_11_Fig43_HTML.jpg)
图 11-43: 外键模式的执行计划

![外键模式的统计 I/O 结果](img/338675_3_En_11_Fig42_HTML.jpg)
...
![计算 JSON 列的执行计划](img/338675_3_En_11_Fig50_HTML.jpg)
图 11-50 计算 JSON 列的执行计划

![计算 JSON 列的统计 I/O 结果](img/338675_3_En_11_Fig49_HTML.jpg)
图 11-49 计算 JSON 列的统计 I/O 结果
...
![计算并创建索引的 JSON 列的执行计划](img/338675_3_En_11_Fig52_HTML.jpg)
图 11-52 计算并创建索引的 JSON 列的执行计划

![计算并创建索引的 JSON 列的统计 I/O 结果](img/338675_3_En_11_Fig51_HTML.jpg)
图 11-51 计算并创建索引的 JSON 列的统计 I/O 结果
...
LEFT JOIN IndexOperationalCTE i2

ON i1.HistoryID = i2.HistoryID + 1
...
为保证完整性，两种方法都将被讨论。在我看来，在 SQL Server 中只应使用`Extended Events`来收集事件追踪数据。这是由于`Extended Events`被很好地集成到 SQL Server 中，有助于防止监控本身引发性能问题。并且它能获取的细节程度远远超出了 `SQL Trace` 的能力。
...
SELECT server_name,
       ...
       FORMAT(1. * SUM(IIF(SearchToScanRatio > 1000, 0, 1)) / COUNT(*), '0.00%') AS PctViolation
FROM CounterSummary
GROUP BY server_name;
```
...
图 14-7：`Lock Wait Time` 计数器分析示例结果
...
SELECT COALESCE(
        DB_NAME([p].[dbid]),
        [p].[query_plan].value('(/ShowPlanXML/BatchSequence/Batch/Statements/StatementSimple/QueryPlan/MissingIndexes/MissingIndexGroup/MissingIndex/@Database)[1]', 'nvarchar(128)')
    ) AS [database_name],
    IIF([p].[objectid] <> 0,
        CONCAT(
