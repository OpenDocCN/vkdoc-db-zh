...
强烈建议您使用索引优化查询，而不是依赖`sort_buffer_size`。您可以考虑增加席 ?? 值，以加快查询优化或改进索引不能改善的 ORT ?? 或 TY3 ?? 操作。
...
另一个用例是提示优化器并节省评估多个计划的成本。考虑下面的 Tyt0}表和查询（与在 Oracle ?? 中讨论的控制查询优化器 OLE T2AY 段的讨论相同）：
...
1.  考虑一下在 Type ?? 中创建的表 ??。第 3 章 AUTT2，AUTT3。
