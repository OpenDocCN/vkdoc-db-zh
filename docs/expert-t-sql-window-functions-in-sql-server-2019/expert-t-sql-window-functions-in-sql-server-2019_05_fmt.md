...
**清单 3-7** 向包含多个表达式的 `GROUP BY` 子句的查询添加窗口聚合
...
**图 3-6** 在包含多个表达式的 `GROUP BY` 查询中使用窗口聚合
