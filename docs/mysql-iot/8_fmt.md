...
1.  使用 `mysqlreplicate` 克隆一个正在运行（已安装）的 MySQL 服务器。至少克隆三到四个副本，并分配正确的端口、服务器 ID、数据库目录等（更改命令中所有出现的 `13001`）。
    ```bash
    ...
    ```
2.  在每个服务器上设置复制用户。使用 `mysql` 客户端依次连接到每个服务器并运行以下 SQL 语句：
    ```sql
    SET @@sql_log_bin=0;
    CREATE USER 'rpl'@'localhost' IDENTIFIED BY 'rpl'
    GRANT REPLICATION SLAVE ON *.* TO 'rpl'@'localhost'
    SET @@sql_log_bin=1;
    ```
3.  一旦所有服务器都克隆完毕，使用 `mysqlreplicate` 设置复制。这里我使用端口 13001 上的服务器作为主服务器。为每个从服务器运行此命令（相应地更改从服务器端口）。
    ```bash
    mysqlreplicate --master=root:root@localhost:13001 --slave=root:root@localhost:13002 --rpl-user=rpl:rpl
    ```
4.  要查看拓扑，请使用 `mysqlrplshow` 并使用服务器和发现从服务器登录选项。
    ```bash
...
1.  确定错误并在参考手册中查找该错误。有时错误是众所周知的或具有可预测的解决方案。
2.  确定导致问题的事件。查看`SHOW SLAVE STATUS`的输出将告诉您事件的二进制日志文件和位置。您可以在主服务器上使用该信息，通过`SHOW BINLOG EVENTS`语句查看事件。如果事件无法运行（错误的表等），则必须跳过该事件。如果您不确定，请重新启动从机并检查错误。
3.  如果必须跳过事件，请发出以下语句来跳过并启动从机。以下语句跳过接下来的两个事件：
    ```
    mysql> SET GLOBAL sql_slave_skip_counter = 2
    mysql> START SLAVE;
    ```
4.  如果这不能解决问题，您可能需要跳过更多事件。但是，如果找不到重新启动点，您可能需要进行更深入的诊断或从最新备份恢复从机并重新启动复制。[<sup>2</sup>](#Fn2)
...
从模块上的错误事务是那些没有从主模块的 UUID 开始的 GTIDs。从这个列表中，我们看到有一个 GTID，`130a0dec-9968-11e5-9766-aa87a4a44672:1`，它是错误的(不在主设备上，也不是源自主设备)。要跳过这一步，我们需要欺骗主节点忽略该事务。让我们看看这些陈述，然后讨论它们的用法。清单 [8-3](#FPar8) 显示了创建一个空事务来忽略(跳过)一个错误事务的过程。

**清单 8-3. 使用 GTID 跳过错误事务**
...
您还看到了如何从文件中读取数据，但不是以将数据插入数据库的方式。但是从机械上来说，这个`insert`与前面展示的`write_data()`方法原语是一样的。因此，我们可以重用这个方法（编写一次，在多个地方使用）。下面显示了该方法的代码：
