...
一旦你成功安装了 setuptools，你就可以打开一个命令提示符并进入 Python 的脚本目录。默认设置为`C:\PythonXY\Scripts\`，其中 XY 代表您的版本号。导航到此位置后，您可以使用前面显示的相同语法来安装 Unix 变体:
...
图 2-4。在 Windows 下安装 PyMongo

### 确认您的 PyMongo 安装可以工作

要确认 PyMongo 安装是否成功完成，可以打开 Python shell。在 Linux 中，你可以通过打开一个控制台并输入`python`来实现。在 Windows 中，您可以通过单击开始➤程序➤ Python XY ➤ Python(命令行)来完成此操作。此时，你将被欢迎进入 Python 的世界(见图 2-5)。
