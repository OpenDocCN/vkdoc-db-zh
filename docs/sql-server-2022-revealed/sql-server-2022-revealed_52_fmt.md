...
这是一个在 Windows 上使用 PowerShell 和 Docker 容器运行时的示例（Windows 上的 Docker Desktop 现在可以使用 Windows Subsystem for Linux，这意味着它作为虚拟化进程运行，而不是一个完整的 Linux 虚拟机）。默认情况下，镜像会缓存在你的本地机器上。如果你运行一个容器而本地没有该镜像，它会首先被拉取到你的本地缓存中。你可以使用 `docker pull` 命令自己拉取镜像。
