...
> **注意**：当您不再需要这些资源时，请务必删除或停止并释放它们，以避免产生任何不必要的费用。
...
> **注意**：要在 Windows 上编辑 hosts 文件，请确保以管理员身份运行您的编辑器。
...
代码清单 2-4 拉取与特定标签关联的容器镜像的 `docker pull` 命令
...
代码清单 2-28：通过 `sqlcmd` 列出所有文件及其物理名称
...
图 2-23：`docker inspect` 的输出 – 卷的详细信息
