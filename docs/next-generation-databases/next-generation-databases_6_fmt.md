...
列式架构的第二个关键优势是压缩。压缩算法主要通过消除数据值内的冗余来工作。高度重复的数据——特别是当这些重复是局部集中时——比重复度低的数据能实现更高的压缩比。尽管无论采用行式还是列式存储，整个数据库中的总重复量是相同的，但压缩方案通常尝试在数据的局部子集上进行；如果压缩能在隔离的数据块上进行，其 CPU 开销会低得多。由于在列式数据库中，列在磁盘上存储在一起，因此实现更高的压缩比所需的计算成本要低得多。
