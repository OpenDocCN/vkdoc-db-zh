...
您可以看到您的类被称为 `DepartmentsImpl`。import 语句表明这意味着 `oracle.jbo.server.EntityImpl`。根据您在“选择 Java 选项”对话框中的选择，JDeveloper 将在类中创建一些占位符方法，您可以在其中添加自己的代码。例如，“数据操作方法”复选框导致 JDeveloper 创建了 `lock()` 和 `doDML()` 方法。
