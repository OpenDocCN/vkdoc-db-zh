...
- 输入模板位置：`/OVS/oda_base_2.8.tar.gz`
- 选择核心许可选项（例如：`16` 个 CPU 核心）
- 设置 ODA Base 域内存（例如：`244` GB）
- 是否分配额外的 VLAN 网络（选择 `n`）
...
- 在两个节点上提取镜像文件
- 配置虚拟机并启动 `oakDom1` 域
- 分配 IP 地址并创建 ODA Base 池
- 启动 ODA Base 并存储配置信息

## 验证部署状态

部署完成后，可以使用以下命令检查虚拟机的状态：
...
`ODA_BASE` 模板只能通过 `dom0` 上的 `vncserver` 进行访问。第一个虚拟机可以通过 VNC 使用端口 `5900` 访问。

```bash
vncsession dom0:5900
```

建立 VNC 会话后，登录主机并使用离线配置工具创建的文件执行部署命令：
