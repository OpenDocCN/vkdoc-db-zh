...
与聚合一样，当使用多个函数时，可能需要多次排序。然而，在分析函数的情况下，每次排序都有自己的操作。比较一下清单 7-9 中两个查询的执行计划。
...
虽然几乎所有的聚合函数都可以作为分析函数运行，但有许多分析函数没有对应的聚合函数。例如，清单 7-10 以一种更易读且更高效的方式重写了清单 7-6。
