...
**注意** `SH.SALES`表是通过嵌套循环连接和一个`TABLE ACCESS BY USER ROWID`操作访问的。这个方法实际上是 11gR2 引入的另一个转换的结果，可以通过在粗体显示的注释中添加一个加号来禁用它。如果禁用了通过嵌套循环进行表查找的转换，第 4 行的`BITMAP CONVERSION TO ROWIDS`操作将成为`TABLE ACCESS BY LOCAL INDEX ROWID BATCHED`操作的子操作，嵌套循环连接就会消失。我不知道通过嵌套循环进行表查找的转换能提供什么好处。
...
代码清单 13-39 表明，即使 CBO 无法处理你编写的因子化子查询，它也能自行生成并妥善使用它们！
