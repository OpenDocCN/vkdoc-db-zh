...
*   `SQL Database` 是一个事务性数据库，通过 `ADO.NET` 或其他提供程序提供熟悉的数据访问方式，并允许您使用标准 `T-SQL` 语句操作数据。`SQL Database` 中的数据库实例有两种版本：`Web` 版和 `Business` 版。`Web` 版提供两种最大数据库大小：`1GB` 和 `5GB`。`Business` 版提供以下最大数据库大小：`10, 20, 30, 40, 50, 100 和 150GB`。
