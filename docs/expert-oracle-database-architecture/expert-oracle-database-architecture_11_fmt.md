...
![段、区段和区块](img/319655_4_En_3_Fig1_HTML.jpg)
...
Oracle 中的临时数据文件（临时文件）是一种特殊类型的数据文件。当内存不足以容纳所有数据时，Oracle 会使用临时文件来存储大型排序操作和哈希操作的中间结果，以及存储全局临时表数据或结果集数据。临时表空间还可以保存在全局临时表上执行操作产生的 `UNDO`。永久数据对象（如表或索引）永远不会存储在临时文件中，但临时表及其索引的内容会存储在其中。因此，你永远不会在临时文件中创建应用程序表，但在使用临时表时，可能会将数据存储在那里。
