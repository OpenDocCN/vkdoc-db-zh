...
![带有溢出段的 IOT，使用了 PCTTHRESHOLD 子句](img/319655_4_En_10_Fig6_HTML.jpg)
...
![带有 OVERFLOW 段和 INCLUDING 子句的 IOT](img/319655_4_En_10_Fig7_HTML.jpg)

**图 10-7**

在这种情况下，无论其中存储的数据大小如何，`Z` 都将在溢出段中离线存储（所有在 `INCLUDING` 子句指定列之后的非主键列都将存储在溢出段中）。

## 选择策略：PCTTHRESHOLD vs. INCLUDING
