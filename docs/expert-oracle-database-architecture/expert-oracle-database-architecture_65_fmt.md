...
我经常看到这种情况——人们理所当然地认为索引总会让查询变得更快。于是，他们建立一个小表，分析它，然后发现优化器没有使用索引。在这种情况下，优化器的做法完全正确。Oracle（在 CBO 下）只会在有意义时才使用索引。考虑以下例子：
...
这些表曾经很小，但现在我们查看时，它们已经变得相当大。现在使用索引是有意义的，而最初则不是。如果我们为表生成统计信息，它就会使用索引。
