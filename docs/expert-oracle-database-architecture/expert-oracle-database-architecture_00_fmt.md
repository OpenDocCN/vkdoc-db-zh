...
要使用 Runstats，你需要设置对几个 `V$` 视图的访问权限，创建一个用于保存统计信息的表，并创建 Runstats 包。创建 Runstats 包的代码包含在 `runstats.sql` 脚本中。你需要访问四个 `V$` 表（那些神奇的动态性能表）：`V$STATNAME`、`V$MYSTAT`、`V$TIMER` 和 `V$LATCH`。以下是我使用的一个视图：
