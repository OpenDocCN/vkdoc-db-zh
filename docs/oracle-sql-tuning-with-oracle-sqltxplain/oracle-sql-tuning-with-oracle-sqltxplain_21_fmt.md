...
有时使用提示是正确的，但有时却是错误的。偶尔，提示是从旧代码中继承下来的，而移除它们以期望性能提升，则需要开发者有足够的勇气。提示也是优化器（CBO）在制定执行计划过程中的一种数据输入形式。之所以常常需要提示，是因为提供给优化器的其他信息是错误的。例如，如果对象统计信息错误，你可能就需要给优化器一个提示，因为它的统计信息是错的。
