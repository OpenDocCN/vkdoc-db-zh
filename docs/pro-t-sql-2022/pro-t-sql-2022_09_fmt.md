...
关于数据如何在`缓冲区缓存`中存储，还有另一点需要考虑。当`SQL Server`执行查询并取回数据时，它只取回存在于`缓冲区缓存`中的数据。从`SQL Server 2019`开始，有一个名为`混合缓冲池`的功能。`混合缓冲池`允许将存储在RAM和持久内存 (PMEM) 中的数据都视为`缓冲区缓存`的一部分。否则，如果数据不在RAM或PMEM中，`SQL Server`将需要访问磁盘来检索数据。检索到的数据随后将被存储在内存中。图 5-3 展示了如何从磁盘检索数据并存储到`缓冲区缓存`中。
