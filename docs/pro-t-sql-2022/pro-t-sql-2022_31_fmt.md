...
根据你想记录的数据类型，一个中间选项可能是使用 `SQL Server 审核`。`SQL Server 审核`可用于跟踪服务器或数据库级别的活动。在记录与应用程序相关的变更的上下文中，你应该专注于数据库审核。`SQL Server 审核`中的数据库审核操作可以跟踪与所有类型数据活动相关的更改，包括访问或修改数据。SQL Server 使用扩展事件来监控审核活动。可检索的数据包括操作发生的时间、触发审核的用户信息以及受影响的对象。也可能记录审核操作发生时所执行的语句。
