...
![图 6-1：树结构示例](img/508094_1_En_6_Fig1_HTML.jpg)
...
![图 6-2：邻接表示例](img/508094_1_En_6_Fig2_HTML.jpg)

一个层级组织结构图，公司总部（Company H Q）`Company Id = 1`，`Reports To = NULL` 位于顶部，有两个子公司：缅因州总部（Maine H Q）`Company Id = 2`，`Reports To = 1` 和田纳西州总部（Tennessee H Q）`Company Id = 3`，`Reports To = 1`，它们各自拥有自己的分公司。
...
![图 6-3：标有路径的示例图结构](img/508094_1_En_6_Fig3_HTML.png)

一个层级组织结构，其中总公司（Company H Q）`Company Id = 1, path= 1` 位于顶端，拥有两个子公司：缅因州总部（Maine H Q）`Company Id = 2, path= 1, 2` 和田纳西州总部（Tennessee H Q）`Company Id = 3, path= 1, 3`，每个子公司又各自拥有其子分支机构。
...
注意：你可以将此方法扩展到任何图结构，甚至是循环图，但它会变得非常庞大和复杂。然而，根据结构的大小和利用率，它可能仍然有价值。
