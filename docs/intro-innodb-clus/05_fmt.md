...
Listing 5-2 Creating Local Server Instances
...
Listing 5-5 Getting the Status of the Cluster
...
## Listing 5-6 Failover Demonstration
...
Listing 5-7 Removing the Downed Instance from the Cluster

### DevOps 脚本

现在，让我们创建 DevOps 脚本，用于设置和关闭 InnoDB 集群。幸运的是，我们在这里不需要发明太多，因为我们将使用在演示中使用的相同命令。唯一的区别是我们将添加`print`语句以获得更好的反馈，并用来自`shell`类的方法替换 shell 命令，该类是 MySQL API 的一部分。
...
Listing 5-8 DevOps Script to Set Up the InnoDB Cluster (Sandbox)
...
Listing 5-9 Setup Script Output
...
**Listing 5-10** DevOps Script to Shut Down InnoDB Cluster (Sandbox)
...
**Listing 5-11** Shutdown Script Output
