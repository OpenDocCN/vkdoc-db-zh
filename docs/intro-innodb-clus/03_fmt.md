...
| `loose-group_replication_group_seeds` | 群组成员列表，用于建立新成员与群组之间的连接。该列表由种子成员的 `group_replication_local_address` 网络地址组成，以逗号分隔的列表形式指定，例如 `host1:port1`，`host2:port2`。 |
...
2018-04-18T17:44:29.133321Z 0 [System] [MY-013170] [Server] /home/cbell/source/git/mysql-bug-staging/build/bin/mysqld (mysqld 8.0.11) initializing of server has completed
...
Listing 3-9 Executing the `setup_gr.sh` Script (Linux)
```
