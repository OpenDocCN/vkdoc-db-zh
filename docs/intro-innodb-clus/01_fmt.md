...
**Listing 1-1** Master Configuration File (Log File and Position)
...
**Listing 1-2** Master Configuration File (GTIDs)
...
**Listing 1-3** Slave Configuration File (Log File and Position)
...
**Listing 1-4** Slave Configuration File (GTIDs)
...
| 主文件中的项目 | 来源 | 例子 |
| --- | --- | --- |
| IP 地址或主机名 | `master.cnf` | `localhost` |
| 港口 | `master.cnf` | `13001` |
| 二进制日志文件 | `SHOW MASTER STATUS` | `master_binlog.000002` |
| 二进制日志文件位置 | `SHOW MASTER STATUS` | `154` |
| 复制用户 ID | `CREATE USER` | `rpl_user` |
| 复制用户密码 | `CREATE USER` | `rpl_pass` |
...
Listing 1-6 Checking SLAVE STATUS
```
...
Listing 1-7 SHOW SLAVE HOSTS Command (Master)
```
...
Listing 1-10 Example Replication Script Output
```
...
Listing 1-11 Example Replication Shutdown Script
```
...
Listing 1-12 Example Replication Shutdown Script Output
```
