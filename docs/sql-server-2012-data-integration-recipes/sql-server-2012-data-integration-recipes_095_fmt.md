...
在映射列时，确保你映射的是已转换的列——名为 `Copy of ColumnName` 或 `DataConversion.ColumnName`。如果你在添加目标任务之前添加数据转换任务，那么映射会稍微复杂一些，因为源列和目标列都会出现在“创建表”对话框中——而你将必须删除重复的（非 Unicode）列。此外，这会使 SSIS 更难映射到正确的输出数据类型。
