...
可能你有很多 XML 片段或文档要加载，并且不想开发一个 SSIS 包来执行此任务。幸运的是，只需一点巧思（和一些动态 SQL），当前配方中使用的脚本就可以扩展为加载多个 XML 片段或文档。加载一系列文件的脚本如下（`C:\SQL2012DIRecipes\CH03\LoadMultiplexmlFiles.sql`）：