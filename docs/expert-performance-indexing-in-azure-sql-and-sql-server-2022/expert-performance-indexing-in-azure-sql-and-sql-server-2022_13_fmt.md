...
一个表格有 6 列，分别标记为 `database_id`、`object_id`、`index_id`、`extent_page_id`、`allocated_page_page_id` 和 `page_type_desc`，以及 2 行，标记为 1 和 2。
...
一个包含两列（标记为 `sales_order_id` 和 `detail_rows`）和 10 行（标记为 1 到 10）的表格。第一行第一列的条目 43697 被高亮显示，而第二列的所有条目均为 1。
...
一个包含 10 行和两列（标记为 `sales_order_id` 和 `detail_rows`）的表格。第一行第一列的条目 43659 被高亮显示。
