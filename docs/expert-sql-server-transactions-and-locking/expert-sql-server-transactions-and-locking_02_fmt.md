...
具有多个键列的索引被称为复合（或组合）索引。复合索引中的数据按照从左到右的列，逐列进行排序。图 1-14 展示了在表的`LastName`和`FirstName`列上定义的复合索引结构。数据首先基于`LastName`（最左列）排序，然后在每个`LastName`值内，再基于`FirstName`排序。
...
表 1-1 复合索引上的 SARGable 和非 SARGable 谓词
...
你可以以一本书为例。页码代表了这本书的聚集索引。书末的索引按字母顺序列出了书中出现的术语列表。每个术语引用了该术语被提及的页码。这代表了术语的非聚集索引。
