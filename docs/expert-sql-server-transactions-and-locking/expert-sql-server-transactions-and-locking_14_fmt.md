...
内存中 OLTP 事务的逻辑结束时间，内存中 OLTP 事务的逻辑开始时间
...
低优先级锁的 `MAX_DURATION` 选项

最大工作线程配置选项

`MEMORY_OPTIMIZED_ELEVATE_TO_SNAPSHOT` 数据库选项
...
`PAGEIOLATCH*` 等待类型，`参见` 等待类型

`PAGELATCH*` 等待类型，`参见` 等待类型
