...
有两个重要的列：作为主键的`ID`，以及代表行加载处理时间的`ProcessingTime`。你应该使用UTC时间而非本地时间，以支持应用服务器位于不同时区的情况，并防止夏令时调整时钟引发问题。该列还有助于防止其他会话在数据仍在处理时重新读取它。最好避免为此目的使用布尔（`bit`）列，因为如果应用服务器崩溃，该行将永远留在表中。而使用时间列，系统可以在超时后再次读取它。
