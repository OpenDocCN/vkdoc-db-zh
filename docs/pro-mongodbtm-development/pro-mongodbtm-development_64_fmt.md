...
另一种用于查找单个文档的方法是重载的 `findOne()`，它具有以下几种变体，在表 10-9 中讨论。
...
在本节中，我们将在 `App` 应用程序的 `findOne()` 方法中，使用 `Query` 对象从 `catalog` 集合中查找一个 `Catalog` 类型的文档。首先，我们需要构造 `Query` 对象来查找该单个文档。`BasicQuery` 类继承自 `Query`，并提供以下构造函数，在表 10-10 中讨论。
...
6.  运行 `App` 应用程序以保存一些 `Catalog` 实例，并使用 `MongoOperations` 中的 `findOne()` 方法查找其中一个 `Catalog` 实例。`findOne()` 方法的输出显示在图 10-12 的 Eclipse 控制台中。
...
当运行 `App` 应用程序时，使用条件定义找到的 `Catalog` 实例中的字段值会输出，如图 10-13 所示。
...
`MongoOperations` 接口提供了重载的 `findAll()` 方法，用于从集合中查找所有文档，如表 10-11 所述。
