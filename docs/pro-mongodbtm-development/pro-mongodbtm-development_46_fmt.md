...
使用命令 `node bulkWriteDocuments.js` 运行脚本。输出如图 5-40 所示。
...
随后在 Mongo shell 中运行 `db.catalog.find()` 命令以列出文档，如图 5-41 所示。
...
我们需要在 `pom.xml` 中添加一些依赖项。按照表 6-1 所列添加以下依赖项；其中一些依赖项已注明随 Apache Cassandra Project 依赖项一同包含，不应单独添加。
