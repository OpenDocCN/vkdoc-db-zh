...
使用 URL `http://localhost:8000/db.php` 在浏览器中运行该 PHP 脚本。为 `local` 数据库列出的集合名称包括 `system.indexes` 和 `catalog`，如图 3-7 所示。不同用户列出的集合可能不同。
...
`createCollection()` 方法中的参数和选项如表 3-4 所示。
