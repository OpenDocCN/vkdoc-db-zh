...
在配置 Spring 环境时，可以使用 `JavaConfig` 来配置普通的旧 Java 对象（POJO）。POJO 是一种普通的 Java 对象，不受 Java 对象模型或约定的任何特殊约束。使用 `JavaConfig` 进行 Spring Data MongoDB 配置的基类是 `org.springframework.data.mongodb.config.AbstractMongoConfiguration`。
