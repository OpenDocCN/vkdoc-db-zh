...
清单 12-13 在 `SELECT` 列表中使用函数会降低性能
...
清单 12-17 类型 `boarding_pass_record`
...
之前我们了解到，对于标量函数，语法 `SELECT * FROM function_name` 和 `SELECT function_name` 之间没有区别。但是当函数返回复合类型时，就存在区别了。
