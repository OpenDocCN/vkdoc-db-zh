...
*清单 14-12 创建 `dbo.RecipeHistoryLog` 表*
...
虽然我们已经创建了文件组、文件、分区函数和分区方案，但这些分区逻辑尚未应用于数据库中的任何数据。我们有两种选择：创建新表或对现有表进行分区。在本示例中，我将从创建新表开始，该表在创建时即进行分区。创建分区表的 T-SQL 代码见清单 15-6。
...
在上面的代码中，最后一行表示该表应创建在清单 15-4 的分区方案上。一旦按照清单 15-6 创建了分区表，表的结构将如图 15-3 所示。

![../images/480547_1_En_15_Chapter/480547_1_En_15_Fig3_HTML.jpg](img/480547_1_En_15_Fig3_HTML.jpg)

在分区表内部，你可以清晰地看到每个分区。在分区表内，你还可以看到分区函数上的正确范围如何将数据分解到各个分区。你可以通过运行清单 15-7 中的查询来确认表是如何分区的。
...
**Listing 15-11** 将所有数据从非分区表切换到分区表
...
![非分区键上的分区视图的执行计划](img/480547_1_En_15_Fig22_HTML.jpg)
