...
16. 您现在应该看到如图 10-10 所示的界面。

![images](img/1010.jpg)

**图 10-10.** 更改区域源的 SQL

17. 应用更改并再次运行报表。请注意，报表显示完整的类别名称而不是类别代码，如图 10-11 所示。
...
19. 浏览图书图片并通过表单上传，如图 10-12 所示。保存对记录的所有更改。

![images](img/1012.jpg)

**图 10-12.** 上传图书图片

20. 要确认图片已成功上传，请再次编辑该记录；这次您会在控件旁边看到一个 `下载` 链接。单击下载链接将带您到上传的图片文件，如图 10-13 所示。
...
5.  完成向导以创建报表。创建完成后，再次编辑该报表。你应该能看到如图 10-14 所示的屏幕。
...
7.  如果你运行报表，应该会看到如图 10-15 所示的标准布局。

    ![images](img/1015.jpg)

    **图 10-15.** 标准报表布局

8.  当然，这个布局是可行的，但它并不是一个非常用户友好的图书目录。如果能以如图 10-16 所示的格式列出每个条目会更好。
...
13. 单击 `Report Attributes` 选项卡。更改列的排序，使 `BookImage` 列首先显示，如图 10-18 所示。

    ![images](img/1018.jpg)

    **图 10-18.** 调整 BOOKIMAGE 列的查看顺序

14. 保存更改并运行报表。现在你应该能看到类似图 10-19 的内容。
...
16. 在列列表中，隐藏除 `BOOKIMAGE` 和 `BOOKTITLE` 列之外的所有列（取消勾选 `Show` 列中的复选框），如图 10-20 所示。
...
20. 现在你应该能看到如图 10-21 所示的屏幕。

    ![images](img/1021.jpg)

    **图 10-21.** 通过 HTML Expression 字段为行定义显示模板

21. 保存并应用更改。现在尝试再次运行报表。你现在应该能看到图书以更用户友好且易读的方式排列，如图 10-22 所示。
...
2. 在 `Security` 区域下，单击 `Security Attributes` 链接，如图 10-23 所示。
...
如第 9 章所述，`APEX` 中指向页面的典型链接具有以下格式：
...
3. 选择 `Pie & Doughnut` 图表类型，如图 10-24 所示。
...
5. 当系统提示你输入图表的 SQL 数据源时，输入以下 SQL：

    ```sql
    SELECT NULL LINK, Category.CategoryName LABEL, SUM(Inventory.CopiesInStock) VALUE FROM Category,Books,Inventory WHERE Category.CategoryID= Books.BookCategory AND Books.BookID=Inventory.BookID GROUP BY Category.CategoryName
    ```

6. 现在你应该能看到如图 10-25 所示的屏幕。

    ![images](img/1025.jpg)

    **图 10-25.** 定义图表的 SQL 数据源

7. 完成向导并运行页面。你应该能立即看到按类别计算的图书总数，如图 10-26 所示。
...
2. 在 `Navigation` 区域，点击图 10-27 所示的 `Lists` 链接。
...
6. 通过点击每个 `Target Page ID or Custom URL` 框旁边的小箭头图标，并从弹出窗口中选择要链接到的正确页面，将每个项目绑定到其各自的报表页面。如图 10-28 所示。
...
8. 在向导的 `Source` 步骤中，选择您之前创建的 `AdministrationOptions` 列表，如图 10-30 所示。
...
10. 现在运行该页面。您应该会看到指向各个管理页面的各种链接，如图 10-31 所示。尝试点击每个链接以确保您能跳转到正确的页面。
...
2. 在 `Navigation` 区域下，点击图 10-32 所示的 `Tabs` 链接。
...
6. 您现在应该看到如图 10-33 所示的屏幕。

    ![images](img/1033.jpg)

    **图 10-33.** 移除所有标准标签页

7. 现在点击 `Manage Tabs` 选项卡，然后点击图 10-34 中高亮显示的小添加按钮。
...
9. 指定 "`Store`" 作为标签页标签，如图 10-35 所示。

    ![images](img/1035.jpg)

    **图 10-35.** 创建 `Store` 标签页

10. 点击 `Next` 按钮。现在您将能够指定与此标签页关联的页面。通过点击图 10-36 中高亮显示的箭头，选择 `My Mini Book catalog` 页面。
