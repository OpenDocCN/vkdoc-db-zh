...
*   所有列必须是原子性的；也就是说，在一个表的单行单列中，只表示一个值。
...
在构建数据库时，你必须注意的最重要的事情之一，就是确保表上有键，以便能够区分不同的行。尽管在 100%的情况下拥有一个完全有意义的键并不总是可行的，但通常情况下这是非常可能的。一个例子是无法区分物理物品本身的场景，比如玉米罐头（或者就此而言，一袋乐高积木）。你无法根据包装区分两个玉米罐头，因此你可能会分配一个作为键的一部分、本身没有意义的值，再加上能将其与其他类似物品（如大罐玉米或小罐菠萝）区分开的信息。不过，一般来说，如果你在现实世界中无法区分两个物品，就不要努力在你的数据库中让它们变得可区分。为货架上的玉米分配一个键，并注明库存罐数，将是完全可接受的。
```
...
图 5-1. 使用自然键的 `driversLicense` 和 `employee` 表
...
图 5-2. 使用代理键的 `driversLicense` 和 `employee` 表
...
图 5-3. 不规范的 `driversLicense` 和 `employee` 表
