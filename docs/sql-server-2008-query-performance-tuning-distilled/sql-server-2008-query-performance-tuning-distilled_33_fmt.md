...
对于一个复杂的查询，需要分析的替代处理策略的数量可能很大，评估每个选项可能需要很长时间。因此，优化器会经历三个不同级别的优化。这些被称为 search 0、search 1 和 search 2。但将它们理解为事务、快速计划和完全优化更容易。根据查询的大小和复杂性，这些不同的优化可能会逐一尝试，或者优化器可能直接跳到完全优化。每种优化都会考虑使用不同的连接技术以及通过扫描、寻道和其他操作访问数据的不同方式。
...
图 15-7 `sys.dm_exec_query_optimizer_info` 的输出
