...
清单 2-5. 为单表构建 XML 的 `FOR XML AUTO` 查询

清单 2-6 展示了此查询的 XML 结果。

```
清单 2-6. 单表 FOR XML AUTO 查询的结果
```
...
清单 2-7. 通过给表取别名来使用 `FOR XML AUTO` 更改元素名称

清单 2-8 展示了使用带别名的源表名执行 `FOR XML AUTO` 查询的 XML 结果。

```
清单 2-8. 源表取了别名的单表 FOR XML AUTO 查询结果
```
...
清单 2-9. 使用 `FOR XML AUTO` 子句构建多个连接表的 XML 数据
...
图 2-3. 当 `FOR XML AUTO` 模式应用于查询中的多个连接表时的分层 XML 结果
