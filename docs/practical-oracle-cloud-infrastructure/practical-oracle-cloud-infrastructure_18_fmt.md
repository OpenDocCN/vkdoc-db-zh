...
![与 OKE 集群相关的子网](img/478313_1_En_8_Fig24_HTML.jpg)

图 8-24: 与 OKE 集群相关的子网
...
列表 8-10 `.kube/sandbox-admin.config`
...
列表 8-12 `sandbox-users.containers.policy.json`
...
列表 9-9 `sandbox-users.policies.storage-reports.json`
...
列表 9-10 `functions.policies.storage-reports.json`
...
```bash
[ubuntu@dev-vm]$ echo "oci" >> ~/reportingfn/requirements.txt
```
...
**清单 9-13** `reportingfn.py: extract_object_name` 函数
...
**图 9-24** GitHub 上的 CloudEvents 项目
