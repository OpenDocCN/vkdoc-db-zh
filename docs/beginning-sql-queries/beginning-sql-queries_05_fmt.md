...
图 5-9. 查找人物及其母亲：**左外连接**的 Access 图示及结果表
...
图 5-10. 查找三代人：**左外连接**的 Access 图示及结果表
...
图 6-5. `Member`表中的外键`Team`和`Team`表中的外键`Manager`，用于表示图 6-4 中的关系
...
图 6-6. 连接`Member`和`Team`以获取关于会员球队的额外信息
...
图 6-7. 连接`Member`和`Team`以获取关于球队经理的额外信息
...
将这个最新的查询和输出与图 6-8 和 6-9 中涉及 `Member` 和 `Team` 表之间单个连接的查询进行比较，你可能会发现这很有指导意义。
