...
对于 InnoDB，开销还取决于二级索引是否适合缓冲池。只要整个索引都在缓冲池中，保持索引更新就相对便宜，不太可能成为严重的瓶颈。如果索引不合适，InnoDB 将不得不在表空间文件和缓冲池之间不断调整页面，这时开销将成为导致严重性能问题的主要瓶颈。
...
[`https://dev.mysql.com/doc/refman/en/fulltext-search-ngram.html`](https://dev.mysql.com/doc/refman/en/fulltext-search-ngram.html)
