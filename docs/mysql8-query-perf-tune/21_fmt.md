...
**清单 21-2** 来自 InnoDB monitor 的事务信息
...
性能模式中用于调查事务的主要表是事务事件表。有三个表格记录当前或最近的事务:`events_transactions_current`、`events_transactions_history`和`events_transactions_history_long`。它们具有表 21-2 中总结的列。
...
Listing 21-8 `events_transactions_summary_global_by_event_name` 表
