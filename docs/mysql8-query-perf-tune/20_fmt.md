...
树格式着重于根据查询各部分之间的关系以及各部分的执行顺序来描述查询是如何执行的。从这个意义上说，它听起来可能类似于 JSON 输出；不过树格式更容易阅读，细节也没那么多。树格式是作为 MySQL 8.0.16 中的一个实验性特性引入的，它依赖于 Volcano 迭代器执行器。从 MySQL 8.0.18 开始，树格式也被用于`EXPLAIN ANALYZER`特性。
...
*   `ordering_operation`：例如，结果为一个`ORDER BY`子句的操作。
...
清单 20-6 The EXPLAIN output for a single table with index lookups
...
清单 20-7 The EXPLAIN output for a simple join between two tables
...
清单 20-8 The EXPLAIN output using part of a multicolumn index
...
[`https://dev.mysql.com/doc/refman/en/explain.html#explain-analyze`](https://dev.mysql.com/doc/refman/en/explain.html%2523explain-analyze)
