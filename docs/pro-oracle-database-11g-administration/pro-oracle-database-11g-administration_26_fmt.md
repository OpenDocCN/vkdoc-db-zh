...
您也可以使用静默安装方法来升级 Oracle 软件的版本。例如，让我们看看如何从 Oracle Database 10g 10.2.0.1 升级到版本 10.2.0.4。
...
修改参数以匹配您的环境。对于此次特定的 Oracle Database 10g 10.2.0.4 升级，以下是最值得注意的需要修改的参数：
...
1. 升级 Oracle 主目录二进制文件。
2. 启动数据库，并运行任何必需的升级脚本。

以下是采用场景 B 方法进行升级的步骤：

1. 保持现有的 Oracle 主目录原样——不对其进行升级。
2. 安装一个与旧 Oracle 主目录版本相同的新 Oracle 主目录。
3. 将新的 Oracle 主目录升级到所需版本。
4. 准备就绪后，关闭使用旧 Oracle 主目录的数据库，设置操作系统变量以指向新的已升级 Oracle 主目录，启动数据库，并运行任何必需的升级脚本。
