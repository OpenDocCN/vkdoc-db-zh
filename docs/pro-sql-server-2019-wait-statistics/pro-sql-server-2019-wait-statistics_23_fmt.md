...
对内存优化表的并发访问通过基于快照的事务隔离处理。该隔离级别与我们可以在基于磁盘的表上使用的快照隔离共享许多特征，但也存在一些差异以优化吞吐量。首先，基于快照的事务隔离在并发事务希望访问同一行时使用行版本。与常规快照隔离将行版本存储在 `TempDB` 数据库中不同，内存优化表的行版本是内联存储在数据文件本身的。
...
动态管理视图（DMVs）：与检测当前等待、阻塞信息查询结果、场景、`sys.dm_exec_sessions`、`sys.dm_os_waiting_tasks`、等待统计流程图、性能监视器（等待统计）、查询存储、`sys.dm_exec_requests`、`sys.dm_exec_session_wait_stats`、`sys.dm_os_waiting_tasks`、`sys.dm_os_wait_stats`。
