...
敏捷性是指能够快速适应不断变化的条件。不仅如此，能够适应和演变以满足变化的条件和需求，对于保持竞争力至关重要。`DBMS`软件本身并不限制你实施变更的能力，但你如何选择使用该软件以及如何设计应用程序却会带来限制。首先，你需要承认并为即将到来的不可避免的变更做好准备。然后，你需要评估项目目标以及你为实现这些目标所拥有的工具。支持敏捷开发流程同时拥有设计良好的数据模型是可能的。使用`PL/SQL`作为`API`可以通过提供一种允许数据库和应用程序演进并支持不断变化的客户需求的方法，来缓冲一些需要进行的变更。
