...
*   表中是否确实存在名为 `order_id` 的列？

*   是否存在名为 `order_id` 的函数？

*   你是否有从该表（或函数）进行选择的权限？

在确保你具备所有必要的先决条件后，Oracle 会生成该 SQL 语句的可执行版本，并将其放入库缓存中。这个过程被称为解析。随后对 SQL 的执行将仅使用已解析的版本，而不再经历解析过程。在执行阶段，Oracle 必须确保组件（表 `ORDERS`、过程 `UPD_QTY` 等）没有被更改。它通过在这些对象以及其他结构（如游标）上放置一个解析锁来实现这一点。
...
注意这些数值。没有任何增加；换句话说，执行过程 `upd_qty` 并未导致解析。在一个不同的会话中，通过添加列来更改表，如下所示：
