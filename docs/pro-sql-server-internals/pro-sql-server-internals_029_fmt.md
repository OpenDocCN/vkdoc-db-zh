...
**全局分配映射（GAM）** 页跟踪区是否已被任何对象分配。数据以位图的形式表示，其中每个位指示一个区的分配状态。零位表示相应的区正在使用中。值为 1 的位表示对应的区是空闲的。每个 `GAM` 页覆盖大约 64,000 个区，即近 4GB 的数据。这意味着每个数据库文件每约 4GB 的文件大小就有一个 `GAM` 页。
...
`SQL Server` 使用另一组称为索引分配映射（`IAM`）的分配映射图页，来跟踪属于某个对象的不同类型页面（`IN_ROW_DATA`、`ROW_OVERFLOW` 和 `LOB` 页）所使用的页和区。每个表/索引都有自己的一组 `IAM` 页，这些页被组合成称为 `IAM` 链的独立链表。每个 `IAM` 链覆盖自己的分配单元——`IN_ROW_DATA`、`ROW_OVERFLOW_DATA` 和 `LOB_DATA`。
