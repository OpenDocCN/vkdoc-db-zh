...
与行溢出数据类似，这里有一个指向另一段信息的指针，称为 `LOB 根结构`，其中包含一组指向其他数据页和行的指针。当 LOB 数据小于 32 KB 并且可以放入五个数据页时，`LOB 根结构`包含指向实际 LOB 数据块的指针。否则，`LOB 树`开始包含额外的中间级指针，类似于索引 B 树，我们将在下一章讨论。
...
存储 `LOB 根结构`的页的 `DBCC PAGE` 命令结果如 清单 1-13 所示。
