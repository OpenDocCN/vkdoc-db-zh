...
你必须被授予 `BACKUP DATABASE` 和 `BACKUP LOG` 权限才能执行备份操作。默认情况下，这些权限授予 `sysadmin` 服务器角色、`db_owner` 和 `db_backupoperator` 数据库角色的成员。此外，SQL Server 启动账户应该具有足够的向指定位置写入备份文件的权限。