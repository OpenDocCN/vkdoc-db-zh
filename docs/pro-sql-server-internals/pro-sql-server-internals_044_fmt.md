...
你还可以通过捕获 `Sort Warning` 和 `Hash Warning` 事件，使用扩展事件和 SQL Server Profiler 来监控 `tempdb` 溢出。此外，SQL Server 2016、SQL Server 2014 SP2 和 SQL Server 2012 SP3 在执行计划中显示与溢出相关的附加信息。它包括并行执行计划中涉及溢出的数据页数量、溢出线程数量，以及内存授予信息。当您需要估算溢出引入的性能影响时，这些信息极为有用。
