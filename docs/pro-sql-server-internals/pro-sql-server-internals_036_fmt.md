...
如图所示，对于`varchar`列，Unicode 字符串参数是非 SARGable 的。这是一个比表面看起来严重得多的问题。虽然你很少像清单 2-8 所示那样编写查询，但如今大多数应用程序开发环境都将字符串视为 Unicode。因此，除非参数数据类型被显式指定为`varchar`。这会使得谓词变为非 SARGable 的，并可能导致严重的性能损失，因为即使`varchar`列上存在索引，也会引发全索引扫描。
