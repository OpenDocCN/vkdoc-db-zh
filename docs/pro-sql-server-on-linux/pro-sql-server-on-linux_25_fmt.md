...
`traceflag`：跟踪标志是在许多不同级别影响 SQL Server 行为的“旋钮”。您将在本书中了解到可用于所有类型的场景。某些跟踪标志需要在 SQL Server 启动时启用，或启用后使其全局应用于所有 SQL Server 会话。对于这些场景，请使用 `mssql-conf` 的 `traceflag` 参数。例如，若希望将关于死锁的诊断信息捕获到 SQL Server 错误日志中，您可以这样开启 `traceflag 1222`：
