...
非聚集索引在`SQL Server 2014 CTP 2`中引入，该版本的文档和白皮书使用术语`range indexes`来指代它们。但是，在`SQL Server 2014`的正式发布版中，Microsoft将术语更改为`nonclustered indexes`。尽管如此，你仍然可以在文档和数据管理视图中找到`range indexes`这个术语。
