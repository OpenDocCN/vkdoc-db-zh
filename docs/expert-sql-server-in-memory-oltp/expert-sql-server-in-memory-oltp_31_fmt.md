...
注意：你可以在 `https://docs.microsoft.com/en-us/sql/relational-databases/resource-governor/resource-governor` 以及我的《Pro SQL Server Internal》一书中阅读更多关于资源调控器的信息。
...
提示：你可以监控 `RESOURCE_SEMAPHORE` 等待、内存授权挂起性能计数器以及 `sys.dm_exec_query_resource_semaphores` 和 `sys.dm_exec_query_memory_grants` 视图，以排查与工作空间内存相关的问题。
...
图 12-7. 来自 `sys.dm_exec_procedure_stats` 视图的数据
