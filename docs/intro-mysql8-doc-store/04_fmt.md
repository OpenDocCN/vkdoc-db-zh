...
与最初的 MySQL 客户端一样，有一些特殊的命令控制应用本身，而不是与数据交互(通过 SQL 或 X DevAPI)。要执行 shell 命令，请发出带斜杠(\)的命令。例如，`\help`打印所有 shell 命令的帮助。表 4-1 列出了一些更常用的 shell 命令。
...
**表 4-2 Common MySQL Shell Options**
...
**Listing 4-1** Connecting with a URI
...
**Listing 4-2** Connecting Using Individual Options
...
如果您一直关注本节中的示例，您可能已经注意到我们使用的端口是 33060。这不是印刷错误。默认情况下，X 插件监听端口 33060，而不是服务器原来默认的端口 3306。事实上，端口 3306 仍然是旧协议的默认端口，您可以使用端口 3306 连接到服务器，但是您必须使用经典会话（`mysqlsh --classic --user root --host localhost --port 3306`）。虽然这表明您可以使用旧的协议连接到服务器，但是回想一下，它确实限制了您可以做的事情，因为 DevAPI 在经典会话对象中并不完全受支持。
...
Listing 4-4 Installing the `world_x` Database in SQL Mode
...
Listing 4-6 Listing and Using Databases—JavaScript Mode
```
