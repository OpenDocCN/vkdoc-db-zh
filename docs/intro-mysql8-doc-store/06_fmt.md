...
**Listing 6-4** Setting Global System Variables
...
**表 6-1** System Variables and Startup Options (X Plugin)
...
| `mysqlx_port` | Thirty-three thousand and sixty | 是 | 不 | 指定 x 插件监听连接的端口 |
...
### 表 6-2 Communication Status Variables (X Plugin)
...
### 表 6-3 Connection Status Variables (X Plugin)
...
### 表 6-4 CRUD Status Variables (X Plugin)
