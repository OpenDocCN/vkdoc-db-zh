...
> **Listing 7-1** Contacts Protobuf Definition
...
`Listing 7-5` Python Generic Error Message (`mysqlx_pb2.proto`)
...
图 7-2 X Protocol connection procedure (Courtesy of Oracle)
...
`Listing 7-6` CapabilitiesSet Message (`mysqlx_connection.proto`)
...
图 7-3 X Protocol simple inserts (Courtesy of Oracle)
...
Listing 7-7 `Sql.StmtExecute` Message (`mysqlx_sql.proto`)
...
**Listing 7-8** Connection Methods for Authenticate Procedure (C/Py)

在 `connect()` 方法中，我们看到发生了几件事情。首先，我们看到 C/Py 打开了一个到服务器的流连接（通过 `_connection_params()` 方法，该方法先前返回了数据集），然后代码创建了一个到读取器/写入器的实例。这就是连接器向/从服务器传输邮件的方式。

接下来，代码实例化 `Protocol` 类的一个实例，这是 X 协议的抽象。稍后我们将看到该代码的更多细节。
...
**Listing 7-9** CapabilitiesGet/Set Methods for Authenticate Procedure (C/Py)
...
Listing 7-10 SQL INSERT Class (C/Py)
```
...
Listing 7-11 The send_insert() Method in the Protocol Class (C/Py)
```
...
我们需要的文件在`/lib/mysqlx`文件夹里。但是首先，在你的系统上创建一个新的文件夹。随便你怎么命名比如`xclient`。接下来，将 C/Py 档案中的`mysqlx`文件夹复制到`xclient`文件夹中。当你为下面的例子创建文件时，把它们保存在`xclient`文件夹中。例如，我将文档存储示例命名为`xclient_json.py`，将关系数据示例命名为`xclient_sql.py`。
...
**Listing 7-13** Definition of the Contacts Collection
...
**Listing 7-14** Results of SELECT Statement for Contacts Collection
...
**Listing 7-15** X Client Source Code (SQL)
