...
**Listing 3-1** Using JSON with SQL Statements
...
`JSON_MERGE_PATCH()`和`JSON_MERGE_PRESERVE()`函数被设计成获取两个或更多 JSON 文档并组合它们。`JSON_MERGE_PATCH()`函数替换重复键的值，而`JSON_MERGE_PRESERVE()`函数保留重复键的值。和前面的函数一样，您可以包含任意多的 JSON 文档。请注意我是如何使用这个函数从前面的例子中构建示例 JSON 文档的。清单 [3-7](#Par126) 展示了一个使用这些方法的例子。
...
Listing 3-10 Using the `JSON_REMOVE` Function (Nested)
...
Listing 3-11 Using the `JSON_REPLACE` Function
...
Listing 3-12 Using the `JSON_SET` Function
...
```sql
SET @base = '{"grapes":["red","white","green"],"berries":["strawberry","raspberry","boysenberry","blackberrry"],"numbers":["1","2","3","4","5"]}';
```

```sql
SELECT JSON_CONTAINS_PATH(@base,'one','$');
```

```sql
SELECT JSON_CONTAINS_PATH(@base,'all','$');
```

```sql
SELECT JSON_CONTAINS_PATH(@base,'all','$.grapes','$.berries');
```

```sql
SELECT JSON_CONTAINS_PATH(@base,'all','$.grapes','$.berries','$.numbers');
```

```sql
SELECT JSON_CONTAINS_PATH(@base,'all','$.grapes','$.berries','$.num');
```

```sql
SELECT JSON_CONTAINS_PATH(@base,'one','$.grapes','$.berries','$.num');
```

```sql
SELECT JSON_CONTAINS_PATH(@base,'one','$.grapes');
```

```sql
SELECT JSON_CONTAINS_PATH(@base,'all','$.grape');
```

```sql
SELECT JSON_CONTAINS_PATH(@base,'one','$.berries');
```

```sql
SELECT JSON_CONTAINS_PATH(@base,'all','$.berries');
```
