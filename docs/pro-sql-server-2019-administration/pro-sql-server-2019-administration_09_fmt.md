...
| `bulkadmin` | 结合对数据库中目标表的 `INSERT` 权限，`bulkadmin` 角色允许用户使用 `BULK INSERT` 语句从文件导入数据。此角色通常授予运行 ETL 过程的服务账户。 |
...
**代码清单 10-23 创建 Chapter10Audit 数据库**
...
![加密层次结构](img/333037_2_En_11_Fig1_HTML.jpg)
...
在主副本发生故障时，故障转移到同步副本是自动的。然而，有些情况下您也需要手动进行故障转移。这可能是因为需要故障转移到灾难恢复站点的灾难，或者是为了主动维护。虽然可以故障转移到异步副本并存在数据丢失的可能性，但良好的做法是首先将数据库置于安全状态。因为如果数据库参与了可用性组，您就不能将其置于 `read_only` 或 `single_user` 模式，所以安全状态通常包括禁用登录账户，然后在故障转移前切换到同步提交模式。
...
表 15-1 `sp_add_log_shipping_primary_database` 参数
...
### 表 15-3 `sp_add_log_shipping_secondary_primary`参数
...
最后一步是重新配置监控，使其能正确监控我们的新配置。我们可以通过使用清单 15-11 中的脚本，为我们的日志传送环境实现这一点。该脚本会连接到主服务器和辅助服务器，因此我们应该在 `SQLCMD` 模式下运行它。
...
![复制组件概览](img/333037_2_En_16_Fig3_HTML.png)
...
![分发服务器页面](img/333037_2_En_16_Fig4_HTML.jpg)
...
![SQL Server 代理启动页面](img/333037_2_En_16_Fig5_HTML.jpg)
...
![代理安全性页面](img/333037_2_En_16_Fig16_HTML.jpg)
...
![快照代理安全性对话框](img/333037_2_En_16_Fig17_HTML.jpg)
...
![发布页面](img/333037_2_En_16_Fig18_HTML.jpg)
...
![初始化订阅页面](img/333037_2_En_16_Fig25_HTML.jpg)
...
![发布访问列表页面](img/333037_2_En_16_Fig26_HTML.jpg)

*图 16-26 发布访问列表页面*

现在，您可以使用 添加 按钮来显示当前没有访问该发布权限的登录名列表。您应从列表中选择适当的登录名将其添加到 `PAL` 中，如图 16-27 所示。

![添加发布访问](img/333037_2_En_16_Fig27_HTML.jpg)
...
一个进程可以获取不同类型的锁。这些锁类型在表 18-4 中描述。
...
意向锁提高了性能，因为它们只在表级被检查，这避免了在其他操作获取锁之前检查每一行或每一页的需要。可以获取的意向锁类型在表 18-5 中描述。
...
表 18-14 `sys.dm_os_waiting_tasks` 列
...
| `内存` | `RESOURCE_SEMAPHORE, CMEMTHREADCMEMPARTITIONED, EE_PMOCKMEMORY_ALLOCATION_EXTRESERVED_MEMORY_ALLOCATION_EXTMEMORY_GRANT_UPDATE` |
...
根据您的具体场景，您可能希望捕获真实的用户活动；为此，您需要使用如`SQLStress`这样的工具，或者编写脚本活动。在我们的情况下，我们编写脚本活动以供跟踪（参见清单 21-9）。
...
图 23-11 创建 `ExcludeSystemDatabases` 条件
...
清单 23-6 使用 `WITH ENCRYPTION` 创建存储过程
...
清单 23-7 使用 T-SQL 检查确保 SQL Server Agent 正在运行
...
清单 23-8 使用 WQL 检查 SQL Server Agent 是否正在运行

要使用 T-SQL 版本的查询，您需要使用`ExecuteSql()`函数，该函数接受表 23-1 中的参数。

表 23-1 `ExecuteSQL()` 参数
