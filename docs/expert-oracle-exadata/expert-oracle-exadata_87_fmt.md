...
顺便说一句，如果您还没有发现这个功能，`cellcli` 会存储类似于 bash shell 的命令历史记录。您可以使用上下箭头键滚动浏览历史记录并使用箭头键编辑命令。多亏了查询中的正则表达式支持，您拥有了一个非常强大的模式匹配能力可供使用。`cellcli` 语法对系统管理员和 DBA 来说可能有些陌生，但一旦您理解了其逻辑，掌握起来真的不难。
