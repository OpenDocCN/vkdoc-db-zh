...
前面清单中显示的输出取自一个运行 Exadata 软件版本 12.1.2.1.0 的单元。如你所见，帮助系统允许你查看每个命令的语法。你可能还注意到了几个 SQL*Plus 的遗留命令。`SET`、`SPOOL` 和 `START` 的功能与预期基本一致。注意 `@` 字符等同于 SQL*Plus 的 `START` 命令，而你能用 `SET` 设置的只有 `ECHO` 和 `DATEFORMAT`。现在，这里有几个使用 `LIST` 命令的查询示例：
