...
那么，哪个是正确的分区键呢？让我们再试一次。`DeviceID`和`Day`作为分区键怎么样(见图 [5-9](#Fig9) )。在这种情况下，数据将有更多的逻辑变化，Azure Cosmos DB 将能够将它们分布到多个物理分区。
...
*   严格在查询条件中使用分区键：计算成本也是选择分区键的一个主要因素。如果您指定一个很少在查询条件中使用的分区键，查询将扇出到所有分区以提供结果。因此，查询成本将变得更高，并导致大量的延迟。假设`deviceidday`作为分区键，请参考图 [5-13](#Fig13) 比较使用和不使用分区键的查询相关成本。

    ![A462104_1_En_5_Fig13_HTML.jpg](img/A462104_1_En_5_Fig13_HTML.jpg)

    图 5-13

    查询成本：左侧是没有分区键的查询（消耗的 RU = 18.43），右侧是使用分区键的查询（消耗的 RU = 7.66）。使用的分区键是`deviceidday`。
