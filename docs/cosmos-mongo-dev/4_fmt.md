...
在`explain()`方法中，在`winningPlan` ➤ `inputStage` ➤ `stage`下，有以下五种可能的操作：
...
如图 [4-1](#Fig1) 所示，使用`IXSCAN`的`winningPlan` ➤ `stage`显示查询正在使用索引。
...
现在，如果你仔细看，这一次我们已经采取了字段`SensorId`，这是没有索引，`winningPlan` ➤ `stage`描述了操作`COLLSCAN`。
...
这些索引用于保存数组值的字段。MongoDB 将为字段中的每个值创建一个索引条目。它可以为标量值（数字、字符串等）或嵌套文档构造。如果 MongoDB 引擎发现字段中有数组或嵌套文档，它会自动创建一个多键索引。因此，语法与复合或单字段索引相同。
...
现在，让我们看看索引的魔力是如何配置的。导航到数据浏览器➤ <<database name>> ➤ <<collection name>> ➤比例&设置。输出如图 [4-2](#Fig2) 所示。
...
默认情况下，`indexingMode`被设置为`consistent`，但是如果您有一个非常繁重的写需求用例，在记录检索中有延迟是可以的，那么您可以使用`lazy`。如果您不必使用查询获取数据，请使用`none`。由于 Azure Cosmos DB 是高性能的，我建议您在更改`indexingMode`之前进行一次负载测试。如有必要，对其进行更改和验证。现在，让我们看看索引路径。
