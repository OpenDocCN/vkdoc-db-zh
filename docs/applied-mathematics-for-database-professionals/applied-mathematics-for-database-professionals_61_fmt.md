...
| t=1 | INSERT | 在序列化锁上被阻塞（`DI`代码的执行被暂停）。 | |
| t=2 | COMMIT | （在 60 秒内。） | |
| t=3 | | `TX4`解除阻塞，`DI`代码恢复执行。 | |
| t=4 | | `DI`代码看到员工 1042 现在管理着三个部门，插入操作失败。 | |
...
你现在不是始终请求名称恒定的应用程序锁（`'DEPT_TAB01'`），而是请求一个名称取决于需要检查的情况的应用程序锁。现在，插入由不同员工管理的两个部门的两个事务被允许同时执行。
