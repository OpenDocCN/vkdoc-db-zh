...
手动压实 SSTable 以重新生成布隆过滤器几乎从来都不是一个好的策略。当您更改表的 `bloom_filter_fp_chance` 属性的值时，您可以改为升级 SSTable。这是通过运行 `nodetool upgradesstables` 命令来完成的。该命令的语法如下：
...
```bash
$ cassandra -Dcassandra.write_survey=true
```
...
`memtable_flush_writers` 参数表示 Memtable 刷新写入线程的数量。数据库使用此参数来计算 `memtable_cleanup_threshold` 参数的值，其默认值由以下方式确定：节点上磁盘的数量或核心的数量，最小值为 2，最大值为 8。
