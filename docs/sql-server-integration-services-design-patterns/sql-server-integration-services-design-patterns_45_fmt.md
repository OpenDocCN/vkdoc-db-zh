...
父-子模式是管理框架的重要组成部分。例如，您可以使用主包模式将类似的包分组在一起，并确保它们按正确的顺序执行。您还可以使用动态子包模式来运行可变数量的包。为了确保存储所有这些信息，在包之间传递信息至关重要，不仅从父到子，也要从子到父。虽然此功能并不广为人知，但使用脚本任务是可以实现的。让我们使用您现有的包来演示如何将文件名从子包传递到其父包。
