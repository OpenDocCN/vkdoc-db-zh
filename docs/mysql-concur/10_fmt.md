...
Listing 10-5 The difference between non-unique and unique secondary indexes
```
...
Listing 10-6 The record locks for Connection 1 (investigation number 1)
```
...
Listing 10-7 The record locks for Connection 2 (investigation number 2)
```
...
清单 10-8Updating a row in a table with foreign keys relationships
...
清单 10-9The InnoDB data locks caused by the UPDATE statement
...
Listing 10-10 The metadata locks caused by the UPDATE statement
```
